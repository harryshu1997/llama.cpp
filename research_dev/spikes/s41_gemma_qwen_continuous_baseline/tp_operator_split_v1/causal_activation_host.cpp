// Host/phone FFN-slice diagnostic.
//
// HTP computes a suffix of gate, up, SiLU, and multiply. The selected host
// backend computes the complementary FFN prefix. The phone returns either the
// gated activation for a host suffix-down continuation or a completed residual
// for a host-side sum.
//
// usage:
...
static bool usb_transfer(
    ...
    if (timeout_value <= 0 || timeout_value > 60000) {
        fprintf(stderr, "[activation-host] invalid AOA timeout\n");
        return false;
    }
...
int main(int argc, char ** argv) {
    ...
    if (shape == "gemma4_12b") {
        k = 3840;
        n_ff = 15360;
    } else if (shape == "gemma4_26b_a4b_top8") {
        k = 2816;
        n_ff = 8 * 704;
    ...
    if (late_path != "f32_sync" &&
        ...
        late_path != "f16_async" &&
        late_path != "host_sum" &&
        late_path != "device_sum") {
        fprintf(stderr, "[activation-host] unknown late path '%s'\n",
                late_path.c_str());
        return 2;
    }
    const bool residual_device_sum =
            residual_output && late_path == "device_sum";
    if ((residual_output &&
         late_path != "host_sum" &&
         late_path != "device_sum") ||
        (!residual_output &&
         (late_path == "host_sum" || late_path == "device_sum"))) {
        fprintf(stderr,
    ...
    if (host_backend_env != nullptr && !host_is_cpu &&
        ...
        strcmp(host_backend_env, "CUDA") != 0) {
        fprintf(stderr, "[activation-host] invalid S41_HOST_BACKEND\n");
        return 2;
    }
    if (host_is_cpu &&
        mode != "host_only" &&
        ...
        mode != "phone_only_gap") {
        fprintf(stderr,
                "[activation-host] CPU supports host_only, host_profile, "
                "spin_corun, phone_first_corun, phone_only, or "
                "phone_only_gap\n");
        return 2;
    }
    if (host_is_cpu && late_dual) {
        fprintf(stderr,
                "[activation-host] f32_dual requires a GPU host backend\n");
        return 2;
    }
    ...
    if (backend == nullptr) {
        fprintf(stderr, "[activation-host] requested backend not found\n");
        return 1;
    }
    int cpu_threads = 0;
    if (host_is_cpu) {
        ...
        if (thread_env != nullptr) {
            ...
            if (end == thread_env || *end != '\0' ||
                ...
                fprintf(stderr, "[activation-host] invalid S41_CPU_THREADS\n");
                return 2;
            }
    ...
    fprintf(stderr,
            "[activation-host] host_backend=%s description=%s "
            "threads=%d\n",
            host_is_cpu ? "CPU" : "GPU",
    ...
    ggml_tensor * returned_residual = ggml_new_tensor_2d(
            context, GGML_TYPE_F32, k, batch);
    ggml_tensor * combined_output =
            ggml_add(
                    context,
                    residual_device_sum
                            ? returned_residual
                            : returned_down_output,
                    prefix_output);
    ...
    if (cache_flush_env != nullptr) {
        ...
        if (end == cache_flush_env || *end != '\0' || parsed > 4096) {
            fprintf(stderr,
                    "[activation-host] invalid S41_HOST_CACHE_FLUSH_MIB\n");
            return 2;
        }
    ...
    if (late_dual) {
        dual_backend = ggml_backend_dev_init(host_device, nullptr);
        ...
        ggml_backend_buffer_type_t host_buffer_type =
                ggml_backend_dev_host_buffer_type(host_device);
    ...
    if (mode == "host_only" ||
        ...
        for (int index = 0; index < iterations; ++index) {
            input_data[0] = s41_activation_value(
                    (uint32_t) output_elements + (uint32_t) index);
            ggml_backend_tensor_set(
                    input, input_data.data(), 0, sizeof(input_data[0]));
            evict_host_cache();
            ...
            output_hashes.push_back(s41_fast_hash_bytes(
                    cuda_only_result.data(),
                    cuda_only_result.size() * sizeof(float)));
        }
        if (mode == "cuda_energy") {
            print_energy_marker("END", "cuda", iterations);
        }
        printf("HOST_ONLY host_backend=%s cache=%s cache_flush_mib=%zu "
               "shape=%s type=%s batch=%d rows=%lld n=%d ",
                host_is_cpu ? "CPU" : "GPU",
        ...
        print_metrics(host_is_cpu ? "cpu_full" : "cuda_full",
                      cuda_only_times);
        double output_l2 = 0.0;
        for (float value : cuda_only_result) {
            output_l2 += (double) value * (double) value;
        }
        const uint32_t final_output_hash = output_hashes.back();
        std::sort(output_hashes.begin(), output_hashes.end());
        const size_t distinct_output_hashes = (size_t) std::distance(
                output_hashes.begin(),
                std::unique(output_hashes.begin(), output_hashes.end()));
        printf("sample=%.9g output_l2=%.9g output_hash=%08x "
               "distinct_output_hashes=%zu/%d non_finite=%zu\n",
                cuda_only_result[0],
                std::sqrt(output_l2),
                final_output_hash,
                distinct_output_hashes, iterations,
                std::count_if(
    ...
    if (mode == "host_profile") {
        std::vector<float> profile_result(output_elements);
        std::vector<double> full_times;
        std::vector<double> prefix_times;
        for (int index = 0; index < 20; ++index) {
            if (ggml_backend_graph_compute(backend, full_graph) !=
                    GGML_STATUS_SUCCESS ||
                ggml_backend_graph_compute(backend, prefix_graph) !=
                    GGML_STATUS_SUCCESS) {
                fprintf(stderr, "[activation-host] profile warmup failed\n");
                return 3;
            }
        }
        for (int index = 0; index < iterations; ++index) {
            ...
            if (ggml_backend_graph_compute(backend, full_graph) !=
                    GGML_STATUS_SUCCESS) {
                fprintf(stderr, "[activation-host] full profile failed\n");
                return 3;
            }
            ggml_backend_tensor_get(
                    full_output, profile_result.data(), 0,
                    profile_result.size() * sizeof(float));
            full_times.push_back(elapsed_ms(started));

            evict_host_cache();
            started = steady_clock::now();
            if (ggml_backend_graph_compute(backend, prefix_graph) !=
                    GGML_STATUS_SUCCESS) {
                fprintf(stderr, "[activation-host] prefix profile failed\n");
                return 3;
            }
            prefix_times.push_back(elapsed_ms(started));
        }
        printf("HOST_PROFILE host_backend=%s cache=%s "
               "cache_flush_mib=%zu shape=%s type=%s batch=%d "
               "phone_cols=%lld n=%d ",
                host_is_cpu ? "CPU" : "GPU",
                cache_flush.empty() ? "warm" : "cold", cache_flush_mib,
                shape.c_str(), ggml_type_name(weight_type), batch,
        ...
        print_metrics(host_is_cpu ? "cpu_full" : "cuda_full",
                      full_times);
        print_metrics(host_is_cpu ? "cpu_prefix" : "cuda_prefix",
                      prefix_times);
        printf("non_finite=%zu\n",
                std::count_if(
                        profile_result.begin(), profile_result.end(),
                        [](float value) { return !std::isfinite(value); }));
        ggml_backend_buffer_free(buffer);
        ggml_free(context);
        ggml_backend_free(backend);
    ...
    auto run_treatment = [&](
        ...
        if (residual_output && !residual_device_sum) {
        ...
        } else if (residual_device_sum) {
            const auto upload_started = steady_clock::now();
            ggml_backend_tensor_set_async(
                    backend, returned_residual,
                    phone_result_data.data(), 0,
    ...
    printf("RESULT host_backend=%s cache=%s cache_flush_mib=%zu "
           "shape=%s type=%s input_type=%s output_type=%s "
    ...
    print_metrics("phone_total", phone_times);
    print_metrics("exposed_wait", exposed_wait_times);
    print_metrics(host_is_cpu ? "late_cpu" : "late_cuda", late_times);
    printf("hidden=%zu/%d hidden_fraction=%.6f\n",
            hidden_count, iterations,
            (double) hidden_count / (double) iterations);
