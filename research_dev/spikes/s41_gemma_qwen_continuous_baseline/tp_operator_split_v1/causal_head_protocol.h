...
static constexpr uint16_t S41_HEAD_PROTOCOL_VERSION = 2;
...
static constexpr uint16_t S41_HEAD_FLAG_TOP_K = 8;
static constexpr uint32_t S41_HEAD_TOP_K = 8;
...
struct s41_head_candidate {
    uint32_t token;
    float score;
};

using s41_head_top1 = s41_head_candidate;
...
static_assert(sizeof(s41_head_candidate) == 8, "unexpected candidate layout");
