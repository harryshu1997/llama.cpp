...
int main(int argc, char ** argv) {
    ...
    const std::string result_name = argc > 7 ? argv[7] : "residual";
    if (result_name != "residual" && result_name != "state") {
        fprintf(stderr, "[attention-worker] unsupported result\n");
        return 2;
    }
    const bool state_output = result_name == "state";
    if (k != 5120 || n_kv <= 0 || group_offset < 0 ||
        group_count <= 0 ||
        group_offset + group_count > S41_QWEN_KV_HEADS) {
        fprintf(stderr,
                "[attention-worker] invalid GQA group range\n");
        return 2;
    }
    ...
    const uint16_t flags = S41_ATTN_FLAG_FAST_HASH |
            ...
            (group_count == 1 ? S41_ATTN_FLAG_SINGLE_GROUP : 0) |
            (state_output
    ...
    parameters.mem_size =
            ggml_tensor_overhead() * 48 + ggml_graph_overhead() * 2;
    ...
    ggml_tensor * output_weight = state_output
            ? nullptr
    ...
    ggml_tensor * k_current = ggml_reshape_3d(
            context, k_output, S41_QWEN_HEAD_DIM, 1, group_count);
    ggml_tensor * v_current = ggml_reshape_3d(
            context, v_output, S41_QWEN_HEAD_DIM, 1, group_count);
    ggml_tensor * key_slot = ggml_view_3d(
            context, key_cache, S41_QWEN_HEAD_DIM, 1, group_count,
            key_cache->nb[1], key_cache->nb[2],
            (size_t) (n_kv - 1) * key_cache->nb[1]);
    ggml_tensor * value_slot = ggml_view_3d(
            context, value_cache, S41_QWEN_HEAD_DIM, 1, group_count,
    ...
    ggml_tensor * attended = ggml_flash_attn_ext(
            ...
            1.0f / std::sqrt((float) S41_QWEN_HEAD_DIM),
            0.0f, 0.0f);
    ggml_flash_attn_ext_set_prec(attended, GGML_PREC_F32);
    ggml_tensor * state =
            ggml_reshape_2d(context, attended, q_width, 1);
    ggml_tensor * result = state_output
            ? state
            : ggml_mul_mat(context, output_weight, state);
    ...
    if (input_buffer == nullptr) {
        fprintf(stderr, "[attention-worker] input allocation failed\n");
        return 1;
    }
    ...
    initialize_cache(
            key_cache, S41_WEIGHT_K,
            (uint32_t) group_offset, (uint32_t) group_count,
            (uint32_t) n_kv);
    initialize_cache(
            value_cache, S41_WEIGHT_V,
            (uint32_t) group_offset, (uint32_t) group_count,
            (uint32_t) n_kv);
    ...
    fprintf(stderr,
            ...
            "attention_mode=group_range result=%s output_type=f16\n",
            ...
            ggml_graph_n_nodes(graph), result_name.c_str());
    fflush(stderr);
    ...
    if (tcp_port_text != nullptr) {
        ...
        if (tcp_port <= 0 || tcp_port > 65535) {
            fprintf(stderr, "[attention-worker] invalid TCP port\n");
            return 2;
        }
        listener = open_tcp_listener((int) tcp_port);
        if (listener < 0) {
            fprintf(stderr, "[attention-worker] TCP listen failed: %s\n",
                    strerror(errno));
            return 1;
        }
        fprintf(stderr, "[attention-worker] TCP port=%ld\n", tcp_port);
        fflush(stderr);
    }
    ...
    for (;;) {
        ...
        if (descriptor < 0) {
            fprintf(stderr, "[attention-worker] endpoint open: %s\n",
                    strerror(errno));
            sleep(1);
            continue;
        }
        ...
        for (;;) {
            ...
            response.output_elements = (uint32_t) output_elements;
