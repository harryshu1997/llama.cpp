...
int main(int argc, char ** argv) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            started = now_us();
            size_t best = 0;
            for (size_t index = 1; index < output_data.size(); ++index) {
                if (output_data[index] > output_data[best]) {
                    best = index;
                }
            }
            const uint32_t reduce_us = elapsed_us(started);
            reduce_times.push_back((double) reduce_us);
            const s41_head_top1 top1 = {
                (uint32_t) offset + (uint32_t) best,
                output_data[best],
            };
            ...
            response.token = top1.token;
            response.score = top1.score;
            ...
            if (!write_exact(descriptor, &response, sizeof(response))) {
                break;
            }
