// CUDA/HTP sharded vocabulary-head diagnostic.
//
// CUDA computes a prefix of vocabulary rows while the phone computes the
...
int main(int argc, char ** argv) {
    ...
    for (size_t index = 0; index < ggml_backend_dev_count(); ++index) {
        ...
        if (ggml_backend_dev_type(device) ==
            ...
            fprintf(stderr, "[head-host] GPU=%s\n",
                    ggml_backend_dev_description(device));
            break;
        }
    }
    if (backend == nullptr) {
        fprintf(stderr, "[head-host] no GPU backend found\n");
        return 1;
    }
    ...
    if (mode == "cuda_only") {
        ...
        printf("HEAD_CUDA_ONLY shape=%s type=%s vocab=%lld n=%d ",
    ...
    s41_head_top1 phone_top = {};
    phone_metrics correctness_phone = {};
    if (!client.run(
                request_id++, input_data, phone_top, correctness_phone)) {
        return 3;
    }
    const s41_head_top1 cuda_partition_top =
            merge_top1(gpu_top, phone_oracle_top);
    const s41_head_top1 split_top = merge_top1(gpu_top, phone_top);
    ...
    for (int index = 0; index < 20; ++index) {
        phone_metrics metrics = {};
        if (!client.run(
                    request_id++, input_data, phone_top, metrics)) {
            return 3;
        }
    }
    if (mode == "phone_only") {
        ...
        printf("token=%u score=%.9g\n", phone_top.token, phone_top.score);
        return 0;
    }
    ...
    printf("HEAD_RESULT shape=%s type=%s input_type=%s "
    ...
    printf("HEAD_VERDICT phone_top1=PASS split_top1=PASS "
