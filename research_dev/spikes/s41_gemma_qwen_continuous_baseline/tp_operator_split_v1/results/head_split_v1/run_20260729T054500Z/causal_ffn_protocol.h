...
static constexpr uint16_t S41_FFN_FLAG_RESIDUAL_OUTPUT = 16;
static constexpr uint16_t S41_FFN_FLAG_MASK = 0x00ffU;
static constexpr uint16_t S41_FFN_BATCH_SHIFT = 8;
static constexpr uint16_t S41_FFN_MAX_BATCH = 255;
...
static inline uint16_t s41_ffn_encode_flags(
        uint16_t flags, uint16_t batch) {
    return (uint16_t) (
            (flags & S41_FFN_FLAG_MASK) |
            (batch > 1 ? batch << S41_FFN_BATCH_SHIFT : 0));
}

static inline uint16_t s41_ffn_decode_batch(uint16_t value) {
    const uint16_t batch = value >> S41_FFN_BATCH_SHIFT;
