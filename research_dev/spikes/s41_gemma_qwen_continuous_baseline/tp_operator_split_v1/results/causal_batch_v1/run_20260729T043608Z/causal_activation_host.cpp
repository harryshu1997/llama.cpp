...
public:
    phone_client(
        ...
        : handle_(handle),
          ...
          output_elements_((size_t) count * batch),
...
int main(int argc, char ** argv) {
    ...
    if (mode == "cuda_profile") {
        ...
        const enum ggml_status oracle_status =
                ggml_backend_graph_compute(
                        backend, oracle_activation_graph);
    ...
    std::vector<float> oracle_activation_data(phone_elements);
    std::vector<float> phone_activation_data(phone_elements);
    ...
    std::vector<float> returned_debug(phone_elements);
    ggml_backend_tensor_get(
    ...
    const size_t activation_argmax_matches = matching_row_argmax(
            oracle_activation_data, phone_activation_data,
            (size_t) phone_columns, batch_size);
    ...
    if (mode == "split_energy") {
        ...
        printf("SPLIT_ENERGY shape=%s type=%s batch=%d rows=%lld "
               "phone_cols=%lld n=%d ",
                shape.c_str(), ggml_type_name(weight_type),
                batch, (long long) iterations * batch,
                (long long) phone_columns, iterations);
