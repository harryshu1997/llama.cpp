// Phone-side gate/up activation worker for the activation-return diagnostic.
//
// The host sends the causal FFN input. HTP computes gate, up, SiLU, and
// multiply for one suffix slice, then returns only the slice activation.
// CUDA retains the matching down-projection weights.
//
// usage:
...
int main(int argc, char ** argv) {
    ...
    const uint16_t request_flags = S41_FFN_FLAG_FAST_HASH |
    ...
    const size_t input_bytes = i8_input
            ...
            : ggml_type_size(input_type) * (size_t) k;
    ...
    if (backend == nullptr) {
        fprintf(stderr, "[activation-worker] backend not found\n");
        return 1;
    }

    ggml_init_params parameters = {};
    parameters.mem_size = ggml_tensor_overhead() * 24 + ggml_graph_overhead();
    ...
    ggml_tensor * gate_output =
            ggml_view_1d(context, gate_up_output, count, 0);
    ggml_tensor * up_output = ggml_view_1d(
    ...
    if (!initialize_gate_up(
                gate_up, weight_type, (uint32_t) offset, (uint32_t) count,
                weight_hash)) {
    ...
    fprintf(stderr,
            ...
            "type=%s input_type=%s output_type=%s "
            "weight_hash=%016llx nodes=%d\n",
    ...
    for (;;) {
        ...
        if (descriptor < 0) {
            fprintf(stderr, "[activation-worker] accessory open: %s\n",
                    strerror(errno));
            sleep(1);
            continue;
        }
        ...
        for (;;) {
            ...
            ggml_backend_tensor_get(
                    activation, output_data.data(), 0,
                    sizeof(float) * (size_t) count);
            get_times.push_back(now_us() - started);

            started = now_us();
            const void * response_output = output_data.data();
            size_t output_bytes = sizeof(float) * (size_t) count;
            if (f16_output) {
                ggml_fp32_to_fp16_row(
                        output_data.data(), output_f16.data(), count);
                response_output = output_f16.data();
                output_bytes = sizeof(ggml_fp16_t) * (size_t) count;
            }
