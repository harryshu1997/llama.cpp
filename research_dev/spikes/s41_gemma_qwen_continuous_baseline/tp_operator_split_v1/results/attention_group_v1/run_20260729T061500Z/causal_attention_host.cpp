...
//
// This bounded static-cache probe checks whether complete GQA groups can run
// concurrently on a phone and CUDA. It does not mutate the resident KV cache.
//
// usage:
...
int main(int argc, char ** argv) {
    ...
    auto finish_graph = [&](attention_path & path) {
        ...
        ggml_build_forward_expand(path.graph, path.k_output);
        ggml_build_forward_expand(path.graph, path.v_output);
    ...
    if (mode == "cuda_only") {
        ...
        printf("sample=%.9g cache_mode=static\n", full_output[0]);
        return 0;
    }
    ...
    printf("hidden=%zu/%d hidden_fraction=%.6f cache_mode=static\n",
