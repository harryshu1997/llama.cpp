...
int main(int argc, char ** argv) {
    ...
    if (k != 5120 || n_kv <= 0 || group_offset < 0 ||
        ...
        fprintf(stderr,
                "[attention-worker] exactly one tail GQA group is supported\n");
        return 2;
    }
    ...
    ggml_tensor * q_heads_tensor = ggml_permute(
            ...
            ggml_reshape_4d(
                    context, q_output,
                    S41_QWEN_HEAD_DIM, S41_QWEN_GQA, 1, 1),
            0, 2, 1, 3);
    ...
    initialize_cache(
            ...
            (uint32_t) group_offset, 1, (uint32_t) n_kv);
    initialize_cache(
    ...
    fprintf(stderr,
            ...
            "attention_mode=single_group result=%s output_type=f16\n",
