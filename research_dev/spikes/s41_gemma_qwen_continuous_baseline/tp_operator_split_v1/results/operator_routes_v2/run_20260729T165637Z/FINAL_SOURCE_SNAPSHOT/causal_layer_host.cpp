...
// CUDA and the phone execute complementary FFN slices. Both sides generate the
// same quantized master weights, and every treatment is checked against a
// monolithic CUDA reference before timing is reported.
//
...
static bool phone_ffn(
        ...
        double & duration_ms) {
    std::vector<ggml_fp16_t> encoded_input((size_t) k);
    ...
    request.reserved = s41_ffn_encode_flags(
            ...
            S41_FFN_FLAG_F16_INPUT |
            S41_FFN_FLAG_F16_OUTPUT |
    ...
    request.count = count;
    request.input_bytes =
            sizeof(ggml_fp16_t) * (uint32_t) encoded_input.size();
    request.input_hash =
            s41_fast_hash_bytes(encoded_input.data(), request.input_bytes);
    ...
    memcpy(
            ...
            encoded_input.data(), request.input_bytes);

    const size_t expected_output_bytes =
            sizeof(ggml_fp16_t) * (size_t) k;
    ...
    if (response.output_hash !=
        s41_fast_hash_bytes(
                encoded_output.data(), expected_output_bytes)) {
...
int main(int argc, char ** argv) {
    ...
    for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
        ...
        if (ggml_backend_dev_type(device) == GGML_BACKEND_DEVICE_TYPE_GPU) {
            ...
            fprintf(stderr, "[causal-host] GPU=%s\n",
                    ggml_backend_dev_description(device));
            break;
        }
    }
    if (backend == nullptr) {
        fprintf(stderr, "[causal-host] no GPU backend found\n");
        return 1;
    }
    ...
    fprintf(stderr,
            ...
            "nodes(monolithic=%d phase_a=%d)\n",
            shape.c_str(), (long long) n_kv, ggml_type_name(weight_type),
            (long long) phone_columns, ggml_graph_n_nodes(monolithic_graph),
    ...
    auto run_split = [&](std::vector<float> & output) {
        ...
        const auto gpu_phase_b_started = steady_clock::now();
        const enum ggml_status launch_status =
                ggml_backend_graph_compute_async(
                        backend, split_phase_b_graph);
        const bool phone_ok = phone_ffn(
                usb, this_request, weight_type, (uint32_t) k, (uint32_t) n_ff,
                (uint32_t) gpu_columns, (uint32_t) phone_columns,
                phone_weight_hash,
                activation_host, phone_output,
                phone_output_f16, phone_ms);
        ggml_backend_synchronize(backend);
        const auto gpu_phase_b_finished = steady_clock::now();
        ...
        last_overlap_span_ms = std::chrono::duration<double, std::milli>(
                gpu_phase_b_finished - gpu_phase_b_started).count();
    ...
    if (mode == "monolithic" || mode == "two_phase") {
        ...
        printf("ENERGY_WINDOW_END unix_ns=%lld mode=%s n=%d\n",
                (long long) unix_time_ns(), mode.c_str(), iterations);
        printf("RESULT control=%s_cuda n=%d median_ms=%.6f p90_ms=%.6f\n",
                mode.c_str(), iterations, percentile(times, 0.5),
    ...
    fprintf(stderr,
            "CORRECTNESS control=two_phase_gpu rel_l2=%.9g max_abs=%.9g "
            "argmax=%zu/%zu non_finite=%zu\n",
            two_phase_metrics.relative_l2, two_phase_metrics.max_absolute,
    ...
    if (phone_columns > 0) {
        ...
        fprintf(stderr,
                "CORRECTNESS control=recursive_cuda_partition rel_l2=%.9g "
                "max_abs=%.9g argmax=%zu/%zu non_finite=%zu\n",
                recursive_split_metrics.relative_l2,
        ...
        fprintf(stderr,
                "CORRECTNESS control=local_cuda_partition rel_l2=%.9g "
                "max_abs=%.9g argmax=%zu/%zu non_finite=%zu\n",
                local_split_metrics.relative_l2, local_split_metrics.max_absolute,
        ...
        fprintf(stderr,
                "CORRECTNESS component=phone_vs_cuda_slice rel_l2=%.9g "
                "max_abs=%.9g argmax=%zu/%zu non_finite=%zu\n",
                phone_partial_metrics.relative_l2,
        ...
        fprintf(stderr,
                "CORRECTNESS treatment=gpu_phone rel_l2=%.9g max_abs=%.9g "
                "argmax=%zu/%zu non_finite=%zu\n",
                split_metrics.relative_l2, split_metrics.max_absolute,
    ...
    if (mode == "split") {
        ...
        printf("ENERGY_WINDOW_END unix_ns=%lld mode=%s n=%d\n",
                (long long) unix_time_ns(), mode.c_str(), iterations);
        printf("RESULT treatment=gpu_phone_isolated n=%d median_ms=%.6f "
               ...
               "result_residency=cuda\n",
                iterations, percentile(split_times, 0.5),
                ...
                percentile(device_ready_times, 0.5));
    ...
    printf("RESULT control=monolithic_cuda n=%d median_ms=%.6f p90_ms=%.6f\n",
            iterations, monolithic_median, percentile(monolithic_times, 0.9));
    printf("RESULT control=two_phase_cuda n=%d median_ms=%.6f p90_ms=%.6f "
           "ratio_vs_monolithic=%.6f\n",
            iterations, two_phase_median, percentile(two_phase_times, 0.9),
            two_phase_median / monolithic_median);
    if (phone_columns > 0) {
        const double split_median = percentile(split_times, 0.5);
        printf("RESULT treatment=gpu_phone n=%d median_ms=%.6f p90_ms=%.6f "
               "phase_a_median_ms=%.6f overlap_span_median_ms=%.6f "
               "phone_median_ms=%.6f late_merge_median_ms=%.6f "
               "device_ready_median_ms=%.6f result_residency=cuda "
               "speedup_vs_monolithic=%.6f "
               "speedup_vs_two_phase=%.6f\n",
                iterations, split_median, percentile(split_times, 0.9),
                percentile(phase_a_times, 0.5),
                percentile(overlap_span_times, 0.5),
                percentile(phone_times, 0.5),
                percentile(late_merge_times, 0.5),
