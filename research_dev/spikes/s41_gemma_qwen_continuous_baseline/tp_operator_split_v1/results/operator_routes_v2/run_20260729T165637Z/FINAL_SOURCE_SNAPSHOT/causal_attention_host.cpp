...
static constexpr int64_t S41_QWEN_GROUP_WIDTH =
        S41_QWEN_GQA * S41_QWEN_HEAD_DIM;
...
static attention_path build_path(
    ...
    path.output = ggml_mul_mat(context, path.wo, path.state);
    path.graph = nullptr;
    return path;
}
...
static bool initialize_path(
    ...
    if (!initialize_weight(
        ...
        !initialize_weight(
                ...
                0, q_row_offset,
                hash_output ? weight_hash : nullptr)) {
        return false;
    }
    initialize_cache(
...
public:
    phone_client(
        ...
        : handle_(handle),
          ...
          state_output_(state_output),
          input_f16_(S41_QWEN_K),
          output_f16_(
                  state_output
                          ? S41_QWEN_GROUP_WIDTH
                          : S41_QWEN_K),
          request_data_(
    ...
    bool run(
        ...
        request.flags = S41_ATTN_FLAG_FAST_HASH |
                ...
                S41_ATTN_FLAG_SINGLE_GROUP |
                (state_output_
                        ? S41_ATTN_FLAG_STATE_OUTPUT
                        : S41_ATTN_FLAG_RESIDUAL_OUTPUT);
        ...
        if (response.magic != S41_ATTN_RESPONSE_MAGIC ||
            ...
            response.output_elements != output_f16_.size() ||
    ...
    const void * encoded_output_data() const {
        return output_f16_.data();
    }

    size_t encoded_output_bytes() const {
        return output_f16_.size() * sizeof(ggml_fp16_t);
    }
...
int main(int argc, char ** argv) {
    ...
    const std::string phone_output_name =
            argc > 6 ? argv[6] : "residual";
    if (phone_output_name != "residual" &&
        phone_output_name != "state") {
        fprintf(stderr, "[attention-host] unknown phone output\n");
        return 2;
    }
    const bool state_output = phone_output_name == "state";
    const int64_t gpu_groups = S41_QWEN_KV_HEADS - phone_groups;
    if (n_kv <= 0 || iterations <= 0 ||
        phone_groups != 1 || gpu_groups <= 0 ||
        weight_type != GGML_TYPE_Q8_0) {
        fprintf(stderr,
                "[attention-host] exactly one phone GQA group is supported\n");
        return 2;
    }
    ...
    ggml_tensor * returned_state = ggml_new_tensor_2d(
            context, GGML_TYPE_F16, S41_QWEN_GROUP_WIDTH, 1);
    ggml_tensor * returned_phone_output =
            ggml_mul_mat(context, oracle.wo, returned_state);
    ggml_tensor * combined_output =
            ggml_add(context, gpu.output, returned_phone_output);
    ...
    const enum ggml_op gpu_output_op = gpu.output->op;
    ggml_tensor * gpu_output_sources[GGML_MAX_SRC];
    memcpy(
            gpu_output_sources, gpu.output->src,
            sizeof(gpu_output_sources));
    gpu.output->op = GGML_OP_NONE;
    memset(gpu.output->src, 0, sizeof(gpu.output->src));
    ggml_build_forward_expand(late_graph, combined_output);
    gpu.output->op = gpu_output_op;
    memcpy(
            gpu.output->src, gpu_output_sources,
            sizeof(gpu_output_sources));
    fprintf(stderr,
            "[attention-host] n_kv=%lld groups=%lld+%lld type=%s "
            "phone_weight_hash=%016llx nodes=%d/%d/%d/%d "
            "cache_mode=last_slot_update phone_output=%s\n",
    ...
    std::vector<float> oracle_state(S41_QWEN_GROUP_WIDTH);
    std::vector<float> phone_result(
            (size_t) (state_output
                    ? S41_QWEN_GROUP_WIDTH
                    : S41_QWEN_K));
    ...
    phone_client client(
            ...
            phone_weight_hash, state_output);
    uint32_t request_id = 1;
    phone_metrics correctness_phone = {};
    if (!client.run(
                request_id++, input_data, phone_result,
                correctness_phone)) {
        return 3;
    }
    if (state_output) {
        ggml_backend_tensor_get(
                oracle.state, oracle_state.data(), 0,
                oracle_state.size() * sizeof(float));
        ggml_backend_tensor_set(
                returned_state, client.encoded_output_data(), 0,
                client.encoded_output_bytes());
        if (!compute(late_graph)) {
            return 3;
        }
        ggml_backend_tensor_get(
                combined_output, split_output.data(), 0,
                split_output.size() * sizeof(float));
    } else {
        for (size_t index = 0; index < split_output.size(); ++index) {
            split_output[index] =
                    gpu_output[index] + phone_result[index];
        }
    }
    ...
    const error_metrics phone_error =
            compare(
                    state_output ? oracle_state : oracle_output,
                    phone_result);
    ...
    for (int index = 0; index < 20; ++index) {
        phone_metrics metrics = {};
        if (!client.run(
                    request_id++, input_data, phone_result, metrics)) {
            return 3;
        }
    }
    std::vector<double> full_times;
    ...
    for (int iteration = 0; iteration < iterations; ++iteration) {
        ...
        if (state_output) {
            auto stage_started = steady_clock::now();
            ggml_backend_tensor_set(
                    returned_state, client.encoded_output_data(), 0,
                    client.encoded_output_bytes());
            upload_times.push_back(elapsed_ms(stage_started));
            stage_started = steady_clock::now();
            if (!compute(late_graph)) {
                return 3;
            }
            late_compute_times.push_back(elapsed_ms(stage_started));
            device_ready_times.push_back(elapsed_ms(total_started));
            stage_started = steady_clock::now();
            ggml_backend_tensor_get(
                    combined_output, split_output.data(), 0,
                    split_output.size() * sizeof(float));
            download_times.push_back(elapsed_ms(stage_started));
        } else {
            auto stage_started = steady_clock::now();
            ggml_backend_tensor_get(
            ...
            download_times.push_back(elapsed_ms(stage_started));
            upload_times.push_back(0.0);
            late_compute_times.push_back(0.0);
            device_ready_times.push_back(elapsed_ms(total_started));
        }
    ...
    if (state_output) {
        print_metrics("device_ready", device_ready_times);
    }
    print_metrics("exposed_wait", exposed_wait_times);
    print_metrics("merge", merge_times);
    printf("hidden=%zu/%d hidden_fraction=%.6f "
           "cache_mode=last_slot_update phone_attention=single_group "
           "phone_output=%s response_bytes=%zu\n",
            hidden_count, iterations,
            (double) hidden_count / (double) iterations,
            phone_output_name.c_str(),
            client.encoded_output_bytes());
    printf("ATTENTION_LATE_BREAKDOWN ");
    print_metrics("upload", upload_times);
    print_metrics("cuda_o_add", late_compute_times);
    print_metrics("download", download_times);
    printf("\n");
