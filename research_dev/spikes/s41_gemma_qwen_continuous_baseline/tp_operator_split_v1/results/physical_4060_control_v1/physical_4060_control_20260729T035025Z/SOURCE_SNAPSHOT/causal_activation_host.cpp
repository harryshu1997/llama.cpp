// CUDA/HTP activation-return diagnostic.
//
// HTP computes a suffix of gate, up, SiLU, and multiply. CUDA computes the
// complementary FFN prefix, then applies the suffix down projection after the
// phone returns the gated activation.
//
// usage:
//   causal_activation_host <iters> <phone_cols> [weight_type] [shape]
//       [input_type] [mode] [output_type] [late_path]
...
public:
    phone_client(
        ...
        : handle_(handle),
          ...
          input_f16_(input_encoding == 1 ? (size_t) k : 0),
    ...
    bool begin(
        ...
        request.reserved = S41_FFN_FLAG_FAST_HASH |
                (input_encoding_ == 1
        ...
        if (input_encoding_ == 1) {
            ...
            for (uint32_t index = 0; index < k_; ++index) {
...
int main(int argc, char ** argv) {
    ...
    if (late_dual &&
        ...
        mode != "split_energy") {
        fprintf(stderr,
                "[activation-host] f32_dual requires steady_corun or "
                "split_energy\n");
        return 2;
    }
    ...
    for (uint32_t index = 0; index < (uint32_t) k; ++index) {
        input_data[index] = s41_activation_value(index);
    }
    ...
    ggml_cgraph * oracle_activation_graph =
            ggml_new_graph_custom(context, 16, false);
    ggml_build_forward_expand(oracle_activation_graph, oracle_activation);
    ...
    if (late_dual) {
        ...
        dual_returned_activation = ggml_new_tensor_2d(
                dual_context, GGML_TYPE_F32, phone_columns, 1);
        ...
        dual_host_buffer = ggml_backend_buft_alloc_buffer(
                host_buffer_type, 2 * (size_t) k * sizeof(float));
        ...
        dual_suffix_host = dual_prefix_host + k;
        if (!initialize_weight(
    ...
    fprintf(stderr,
            ...
            "output_type=%s late_path=%s nodes=%d/%d/%d\n",
            ...
            output_type.c_str(), late_path.c_str(),
    ...
    if (mode == "cuda_only" || mode == "cuda_energy") {
        std::vector<float> cuda_only_result((size_t) k);
        ...
        printf("CUDA_ONLY shape=%s type=%s n=%d ",
                shape.c_str(), ggml_type_name(weight_type), iterations);
    ...
    std::vector<float> full_result((size_t) k);
    std::vector<float> local_result((size_t) k);
    std::vector<float> treatment_result((size_t) k);
    std::vector<float> dual_suffix_result((size_t) k);
    std::vector<float> oracle_activation_data((size_t) phone_columns);
    std::vector<float> phone_activation_data((size_t) phone_columns);

    if (!compute(full_graph) ||
        !compute(local_partition_graph) ||
        !compute(oracle_activation_graph)) {
        return 3;
    }
    ggml_backend_tensor_get(
    ...
    ggml_backend_tensor_get(
            oracle_activation, oracle_activation_data.data(), 0,
            oracle_activation_data.size() * sizeof(float));
    ...
    auto run_treatment = [&](
        ...
        if (mode == "phone_first_corun") {
            ...
            if (!client.finish(
                        current_request_id, phone_activation_data,
                        metrics.phone)) {
                return false;
            }
            ...
            const auto submitted = spin_client->submit(
                    request_id++, input_data, phone_activation_data);
        ...
        const void * late_data = late_f16
                ...
                : phone_activation_data.data();
        const size_t late_bytes = late_f16
                ? client.encoded_output_bytes()
                : phone_activation_data.size() * sizeof(float);
        if (late_dual) {
    ...
    const error_metrics activation_error =
            compare(oracle_activation_data, phone_activation_data);
    ...
    printf("CORRECTNESS component=phone_activation rel_l2=%.9g "
    ...
    printf("CORRECTNESS treatment=activation_return rel_l2=%.9g "
           "max_abs=%.9g argmax=%zu/%zu non_finite=%zu\n",
    ...
    printf("BOUNDARY phone_sample=%.9g tensor_sample=%.9g "
           ...
           "combined_sample=%.9g reference_sample=%.9g\n",
            phone_activation_data[0], returned_debug[0],
            oracle_activation_data[0], prefix_debug[0], suffix_debug[0],
    ...
    if (mode == "phone_only" || mode == "phone_only_gap") {
        ...
        printf("PHONE_ONLY shape=%s type=%s input_type=%s output_type=%s "
               ...
               "phone_cols=%lld n=%d ",
                ...
                input_type.c_str(), output_type.c_str(), mode.c_str(),
    ...
    if (mode == "split_energy") {
        ...
        printf("SPLIT_ENERGY shape=%s type=%s phone_cols=%lld n=%d ",
                shape.c_str(), ggml_type_name(weight_type),
                (long long) phone_columns, iterations);
    ...
    printf("RESULT shape=%s type=%s input_type=%s output_type=%s mode=%s "
           "late_path=%s "
           "phone_cols=%lld n=%d ",
            shape.c_str(), ggml_type_name(weight_type), input_type.c_str(),
            output_type.c_str(),
            mode.c_str(), late_path.c_str(),
            (long long) phone_columns, iterations);
