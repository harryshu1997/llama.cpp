...
contract=${HANDOFF_CONTRACT:-$here/W5_HANDOFF_CONTRACT.json}
...
out=${1:-$here/results/w5_phone_cuda_handoff/run_$stamp}

phone_head_port=41315
phone_tail_port=41332
phone_relay_port=41325
cuda_head_port=41310
cuda_tail_port=41311
cuda_relay_port=41312
...
collect_phone_logs() {
    ...
    adb -P "$adb_port" -s "$op15" pull \
        "$runtime/w5_handoff_head.log" "$out/op15_head.log" \
        >/dev/null 2>&1 || true
    adb -P "$adb_port" -s "$op15" pull \
        "$runtime/w5_handoff_relay.log" "$out/phone_relay.log" \
        >/dev/null 2>&1 || true
    adb -P "$adb_port" -s "$op12" pull \
        "$runtime/w5_handoff_tail.log" "$out/op12_tail.log" \
        >/dev/null 2>&1 || true
}
...
cleanup() {
    ...
    if (( phone_started )); then
        collect_phone_logs
        stop_remote_pid "$op15" w5_handoff_relay.pid
        stop_remote_pid "$op15" w5_handoff_head.pid
        stop_remote_pid "$op12" w5_handoff_tail.pid
    fi
    local pid
...
probe_sha=$(sha256sum "$here/phone_cuda_handoff_probe.py" | awk '{print $1}')
validator_sha=$(sha256sum "$here/validate_phone_cuda_handoff.py" | awk '{print $1}')
...
value = {
    ...
    "sources": {
        ...
        "phone_cuda_handoff_probe.py": "$probe_sha",
        "qwen25_quality_probe.py": "$quality_probe_sha",
        "run_w5_handoff_gate.sh": "$launcher_sha",
        "stage_v3_client.py": "$stage_client_sha",
        "validate_phone_cuda_handoff.py": "$validator_sha",
    },
}
...
adb -P "$adb_port" -s "$op12" shell "
    cd '$runtime'
    rm -f w5_handoff_tail.log w5_handoff_tail.pid
    nohup env LD_LIBRARY_PATH=. ADSP_LIBRARY_PATH=. \
        ...
        >'$runtime/w5_handoff_tail.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w5_handoff_tail.pid'
"
wait_phone_log "$op12" w5_handoff_tail.log \
    "[stagenet] listening on 0.0.0.0:$phone_tail_port" \
    w5_handoff_tail.pid

adb -P "$adb_port" -s "$op15" shell "
    cd '$runtime'
    rm -f w5_handoff_head.log w5_handoff_head.pid
    nohup env LD_LIBRARY_PATH=. ADSP_LIBRARY_PATH=. \
        ...
        --driver-batch 8 --driver-context 32 --driver-max-prefill 8 \
        --devices GPUOpenCL -ngl 99 \
        >'$runtime/w5_handoff_head.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w5_handoff_head.pid'
"
wait_phone_log "$op15" w5_handoff_head.log \
    "[stagenet] listening on 0.0.0.0:$phone_head_port" \
    w5_handoff_head.pid

adb -P "$adb_port" -s "$op15" shell "
    cd '$runtime'
    rm -f w5_handoff_relay.log w5_handoff_relay.pid
    nohup ./llama-stage-direct-relay \
        --listen '$phone_relay_port' \
        --head '127.0.0.1:$phone_head_port' \
        --tail '$op12_wifi:$phone_tail_port' \
        >'$runtime/w5_handoff_relay.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w5_handoff_relay.pid'
"
wait_phone_log "$op15" w5_handoff_relay.log \
    "[direct-relay] listening on 0.0.0.0:$phone_relay_port" \
    w5_handoff_relay.pid
...
set +e
PYTHONDONTWRITEBYTECODE=1 python3 "$here/phone_cuda_handoff_probe.py" \
    --contract "$contract" \
    --phone-route "$op15_wifi:$phone_relay_port" \
    --cuda-route "127.0.0.1:$cuda_relay_port" \
    --timeout 300 \
    --phone-session-end stop \
    --output "$out/handoff_report.json" \
...
wait_phone_exit "$op15" w5_handoff_relay.pid || phone_exit_ok=0
wait_phone_exit "$op15" w5_handoff_head.pid || phone_exit_ok=0
wait_phone_exit "$op12" w5_handoff_tail.pid || phone_exit_ok=0
collect_phone_logs

if [[ -f $out/handoff_report.json ]]; then
    set +e
    PYTHONDONTWRITEBYTECODE=1 python3 \
        ...
        --output "$out/handoff_certificate.json" \
...
value = {
    ...
    "schema": "s39-phone-cuda-handoff-exit-codes-v1",
...
if (( probe_rc != 0 || validator_rc != 0 ||
    ...
    printf 'W5_HANDOFF_GATE=FAIL probe_rc=%d validator_rc=%d host_exit=%d phone_exit=%d output=%s\n' \
        "$probe_rc" "$validator_rc" "$host_exit_ok" "$phone_exit_ok" "$out" >&2
    exit 3
fi
printf 'W5_HANDOFF_GATE=PASS output=%s\n' "$out"
