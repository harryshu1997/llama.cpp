...
contract=${PROMOTION_CONTRACT:-$here/W7_COLD_PROMOTION_CONTRACT_R1.json}
...
out=${1:-$here/results/w7_cold_promotion_r1/run_$stamp}
...
collect_phone_logs() {
    ...
    adb -P "$adb_port" -s "$op15" pull \
        "$runtime/w7_promotion_head.log" "$out/op15_head.log" \
        >/dev/null 2>&1 || true
    adb -P "$adb_port" -s "$op15" pull \
        "$runtime/w7_promotion_relay.log" "$out/phone_relay.log" \
        >/dev/null 2>&1 || true
    adb -P "$adb_port" -s "$op15" pull \
        "$runtime/w7_warmup_relay.log" "$out/phone_warmup_relay.log" \
        >/dev/null 2>&1 || true
    adb -P "$adb_port" -s "$op12" pull \
        "$runtime/w7_promotion_tail.log" "$out/op12_tail.log" \
        >/dev/null 2>&1 || true
}
...
cleanup() {
    ...
    if (( phone_started )); then
        collect_phone_logs
        stop_remote_pid "$op15" w7_warmup_relay.pid
        stop_remote_pid "$op15" w7_promotion_relay.pid
        stop_remote_pid "$op15" w7_promotion_head.pid
        stop_remote_pid "$op12" w7_promotion_tail.pid
    fi
    local pid
...
[[ -f $contract ]] || fail "missing promotion contract: $contract"
...
promotion_probe_sha=$(sha256sum \
    "$here/phone_cuda_cold_promotion_probe.py" | awk '{print $1}')
control_probe_sha=$(sha256sum \
    "$here/cuda_cold_control_probe.py" | awk '{print $1}')
preparation_probe_sha=$(sha256sum \
    "$here/prepare_phone_route.py" | awk '{print $1}')
delta_probe_sha=$(sha256sum "$here/phone_cuda_delta_probe.py" | awk '{print $1}')
handoff_probe_sha=$(sha256sum \
    "$here/phone_cuda_handoff_probe.py" | awk '{print $1}')
validator_sha=$(sha256sum "$here/validate_cold_promotion.py" | awk '{print $1}')
...
value = {
    ...
    "run_id": "$run_id",
    "schema": "s39-cold-promotion-context-v1",
    "sources": {
        "async_pipeline.py": "$endpoint_parser_sha",
        "cuda_cold_control_probe.py": "$control_probe_sha",
        "phone_cuda_cold_promotion_probe.py": "$promotion_probe_sha",
        "phone_cuda_delta_probe.py": "$delta_probe_sha",
        "phone_cuda_handoff_probe.py": "$handoff_probe_sha",
        "prepare_phone_route.py": "$preparation_probe_sha",
        "qwen25_quality_probe.py": "$quality_probe_sha",
        "run_w7_cold_promotion_gate.sh": "$launcher_sha",
        "stage_v3_client.py": "$stage_client_sha",
        "validate_cold_promotion.py": "$validator_sha",
...
adb -P "$adb_port" -s "$op12" shell "
    cd '$runtime'
    rm -f w7_promotion_tail.log w7_promotion_tail.pid
    nohup env LD_LIBRARY_PATH=. ADSP_LIBRARY_PATH=. \
        ...
        >'$runtime/w7_promotion_tail.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w7_promotion_tail.pid'
"
wait_phone_log "$op12" w7_promotion_tail.log \
    "[stagenet] listening on 0.0.0.0:$phone_tail_port" \
    w7_promotion_tail.pid

adb -P "$adb_port" -s "$op15" shell "
    cd '$runtime'
    rm -f w7_promotion_head.log w7_promotion_head.pid
    nohup env LD_LIBRARY_PATH=. ADSP_LIBRARY_PATH=. \
        ...
        --driver-batch 8 --driver-context 32 --driver-max-prefill 8 \
        --devices GPUOpenCL -ngl 99 \
        >'$runtime/w7_promotion_head.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w7_promotion_head.pid'
"
wait_phone_log "$op15" w7_promotion_head.log \
    "[stagenet] listening on 0.0.0.0:$phone_head_port" \
    w7_promotion_head.pid

adb -P "$adb_port" -s "$op15" shell "
    cd '$runtime'
    rm -f w7_warmup_relay.log w7_warmup_relay.pid
    nohup ./llama-stage-direct-relay \
        --listen '$phone_relay_port' \
        --head '127.0.0.1:$phone_head_port' \
        --tail '$op12_wifi:$phone_tail_port' \
        >'$runtime/w7_warmup_relay.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w7_warmup_relay.pid'
"
wait_phone_log "$op15" w7_warmup_relay.log \
    "[direct-relay] listening on 0.0.0.0:$phone_relay_port" \
    w7_warmup_relay.pid

PYTHONDONTWRITEBYTECODE=1 python3 \
    "$here/prepare_phone_route.py" \
    --contract "$contract" \
    ...
    --run-id "$run_id" \
    --output "$out/phone_preparation_report.json" \
    --timeout 300 \
    >"$out/phone_preparation.stdout" \
    2>"$out/phone_preparation.stderr"
wait_phone_exit "$op15" w7_warmup_relay.pid ||
    fail "phone preparation relay did not exit"
adb -P "$adb_port" -s "$op15" shell \
    "rm -f '$runtime/w7_warmup_relay.pid'"

adb -P "$adb_port" -s "$op15" shell "
    cd '$runtime'
    rm -f w7_promotion_relay.log w7_promotion_relay.pid
    nohup ./llama-stage-direct-relay \
        --listen '$phone_relay_port' \
        --head '127.0.0.1:$phone_head_port' \
        --tail '$op12_wifi:$phone_tail_port' \
        >'$runtime/w7_promotion_relay.log' 2>&1 </dev/null &
    echo \$! >'$runtime/w7_promotion_relay.pid'
"
wait_phone_log "$op15" w7_promotion_relay.log \
    "[direct-relay] listening on 0.0.0.0:$phone_relay_port" \
    w7_promotion_relay.pid
...
wait_phone_exit "$op15" w7_promotion_relay.pid || phone_exit_ok=0
wait_phone_exit "$op15" w7_promotion_head.pid || phone_exit_ok=0
wait_phone_exit "$op12" w7_promotion_tail.pid || phone_exit_ok=0
collect_phone_logs
...
if (( treatment_probe_rc == 0 && host_exit_ok == 1 &&
    ...
    PYTHONDONTWRITEBYTECODE=1 python3 \
        "$here/cuda_cold_control_probe.py" \
        --contract "$contract" \
        --base-contract "$base_contract" \
        --delta-contract "$delta_contract" \
        --physical-gate "$physical_gate" \
...
if (( treatment_probe_rc == 0 && control_probe_rc == 0 &&
   ...
   [[ -f $out/control_report.json ]]; then
    set +e
    PYTHONDONTWRITEBYTECODE=1 python3 \
        "$here/validate_cold_promotion.py" \
        --contract "$contract" \
        --base-contract "$base_contract" \
        --delta-contract "$delta_contract" \
        --physical-gate "$physical_gate" \
        --preparation-report "$out/phone_preparation_report.json" \
        --treatment-report "$out/treatment_report.json" \
...
value = {
    ...
    "schema": "s39-cold-promotion-exit-codes-v1",
...
if (( treatment_probe_rc != 0 || control_probe_rc != 0 ||
    ...
    printf 'W7_COLD_PROMOTION_GATE=FAIL treatment_rc=%d control_rc=%d validator_rc=%d host_exit=%d phone_exit=%d output=%s\n' \
        "$treatment_probe_rc" "$control_probe_rc" "$validator_rc" \
        "$host_exit_ok" "$phone_exit_ok" "$out" >&2
    exit 3
fi
printf 'W7_COLD_PROMOTION_GATE=PASS output=%s\n' "$out"
