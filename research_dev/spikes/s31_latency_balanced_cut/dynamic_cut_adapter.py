#!/usr/bin/env python3
"""Build the S31 R0/R2 topology for a measured phone layer cut."""
...
from stage_v3_client import StageV3Client, require_same_model


DEFAULT_KNEES = {name: 32 for name in WORKER_NAMES}
PHONE_OUTPUT_LAYER = 8


def phone_cut(args: Any) -> int:
    value = getattr(args, "phone_cut", None)
    if type(value) is not int or not 1 <= value < PHONE_OUTPUT_LAYER:
        raise PhysicalRuntimeError("phone cut must be an integer in [1, 8)")
    return value


def expected_routes(cut: int) -> dict[str, tuple[tuple[str, int, int], ...]]:
    if type(cut) is not int or not 1 <= cut < PHONE_OUTPUT_LAYER:
        raise PhysicalRuntimeError("phone cut must be an integer in [1, 8)")
    return {
        "R0": (
            ("cuda-prefix", 0, 6),
...
def validate_shared_pair(routes: Sequence[FiniteRoute], cut: int) -> None:
    expected_routes_by_id = expected_routes(cut)
    by_id = {route.route_id: route for route in routes}
    if len(by_id) != len(routes) or set(by_id) != set(expected_routes_by_id):
        raise PhysicalRuntimeError("topology must define exactly R0 and R2")
    for route_id, expected in expected_routes_by_id.items():
        actual = tuple(
            (stage.worker_name, stage.layer_start, stage.layer_end)
            for stage in by_id[route_id].stages
        )
        if actual != expected:
            raise PhysicalRuntimeError(f"{route_id} differs from the selected S31 cut")
...
def build_topology(args: Any) -> PhysicalTopology:
    ...
    try:
        ...
        require_same_model(hellos, expected_file_type=7)
        for name in WORKER_NAMES:
        ...
        def stage(worker: str, start: int, end: int, terminal: bool) -> ResidentStage:
        ...
        op12_prefix = stage("op12-prefix", 0, cut, False)
        op15_mid = stage("op15-mid", cut, 8, False)
        ...
        validate_shared_pair(routes, cut)
