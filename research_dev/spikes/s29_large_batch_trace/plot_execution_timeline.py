#!/usr/bin/env python3
"""Plot the persisted S29 request and physical-batch timelines."""
...
PLACEMENTS = {
    "R0": "4060 [0,6) -> 4060 [6,8) -> 4060 [8,48)",
    "R2": "OP12 [0,6) -> OP15 [6,8) -> 4060 [8,48)",
}
ROUTE_COLORS = {"R0": "#d94841", "R2": "#138a72"}
PRIORITY_COLORS = {0: "#c7352d", 1: "#e29b22", 2: "#3574b9"}
WORKERS = (
    ("cuda-prefix", "4060 [0,6)"),
    ("cuda-mid", "4060 [6,8)"),
    ("op12-prefix", "OP12 [0,6)"),
    ("op15-mid", "OP15 [6,8)"),
    ("cuda-tail", "4060 [8,48)"),
)
...
def load_report(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise PlotError(f"{path} is not a complete S29 report")
    return value
...
def write_request_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    fields = (
        "request_index", "request_id", "priority", "route_id", "placement",
        "arrival_s", "admitted_s", "first_token_s", "completed_s",
        "latency_s", "slo_s", "slo_met",
    )
...
def request_panel(axis: Any, rows: list[dict[str, Any]], max_time: float) -> None:
    for y, row in enumerate(rows):
        ...
        route = row["route_id"]
        axis.barh(
            y, completed - arrival, left=arrival, height=0.72,
            color=ROUTE_COLORS[route], alpha=0.86, linewidth=0,
        )
        axis.plot(
            first_token, y, marker="o", markersize=2.8, color="#111111",
            linestyle="none", zorder=3,
        )
        axis.plot(
            min(deadline, max_time), y, marker="|", markersize=5,
            color="#222222", linestyle="none", zorder=3,
        )
    axis.set_yticks(range(len(rows)))
    axis.set_yticklabels([
        f'{index:02d}  P{row["priority"]}  {row["route_id"]}'
        for index, row in enumerate(rows, 1)
    ], fontsize=6)
    axis.invert_yaxis()
    ...
    axis.set_title(
        "A. Real request placement: arrival to completion",
    ...
    axis.legend(
        handles=[
            Patch(color=ROUTE_COLORS["R0"], label="R0: all CUDA"),
            Patch(color=ROUTE_COLORS["R2"], label="R2: OP12 -> OP15 -> CUDA"),
            Line2D([], [], color="#111111", marker="o", linestyle="none", label="first token"),
            Line2D([], [], color="#222222", marker="|", linestyle="none", label="SLO (clipped)"),
        ],
        loc="lower right", fontsize=8, frameon=True, ncol=2,
    )


def device_panel(axis: Any, report: dict[str, Any], max_time: float) -> None:
    ...
    for lane, (worker, _label) in enumerate(WORKERS):
        for event in report["batch_events"][worker]:
            ...
            height = 0.20 + 0.70 * event["batch_size"] / 32.0
            axis.broken_barh(
                [(start, duration)], (lane - height / 2, height),
                facecolors=PRIORITY_COLORS[min(event["priorities"])],
    ...
    axis.set_xlabel("Seconds from trace start")
    axis.set_title(
        "B. Actual device batches (bar height scales from B1 to B32)",
        loc="left", fontsize=12, fontweight="bold",
    )
    axis.grid(axis="x", color="#dddddd", linewidth=0.6)
    axis.set_axisbelow(True)
...
def benefit_panel(axis: Any, control: dict[str, Any], treatment: dict[str, Any]) -> None:
    ...
    labels = ("Summed CUDA\ncompute", "P0 p95\nlatency", "Makespan")
    bars = axis.barh(range(3), ratios, color=("#138a72", "#3574b9", "#d94841"), height=0.55)
    axis.axvline(100, color="#222222", linewidth=1.0, linestyle="--")
    axis.set_yticks(range(3), labels=labels)
    axis.invert_yaxis()
    axis.set_xlim(0, max(ratios) * 1.15)
    axis.set_xlabel("Treatment as percent of all-CUDA control")
    axis.set_title("C. Benefit and cost (lower is better)", loc="left", fontsize=12, fontweight="bold")
    axis.grid(axis="x", color="#dddddd", linewidth=0.6)
    axis.set_axisbelow(True)
    ...
    phone_sizes = [
        ...
        for event in treatment["batch_events"][worker]
    ]
    axis.text(
        0.99, 0.96,
        f"60/60 complete, 0 SLO misses; phone batches B{min(phone_sizes)}-B{max(phone_sizes)}\n"
        f"CUDA compute {(ratios[0] - 100):+.2f}%; P0 p95 {(ratios[1] - 100):+.2f}%; "
        f"makespan {ratios[2] / 100:.2f}x\n"
        "Phone/network energy unknown; F16-phone/Q8-server accuracy uncertified",
        transform=axis.transAxes, ha="right", va="top", fontsize=8.5,
        bbox={"facecolor": "white", "edgecolor": "#bbbbbb", "pad": 5},
    )


def plot(control: dict[str, Any], treatment: dict[str, Any], png: Path, svg: Path, csv_path: Path) -> None:
    rows = request_rows(treatment)
    write_request_csv(csv_path, rows)
    max_time = max(row["completed_ns"] for row in rows) / 1e9 * 1.04
    ...
    figure.suptitle(
        "S29 real B32 trace: priority-aware phone offload\n"
        "RTX 4060 Ti + OP12 + OP15 | one-second online P0 quiet guard",
        fontsize=16, fontweight="bold", linespacing=1.45,
    )
    figure.text(
        0.5, 0.006,
...
def main() -> int:
    ...
    plot(
        load_report(args.control), load_report(args.treatment),
