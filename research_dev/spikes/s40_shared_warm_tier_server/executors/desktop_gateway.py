...
class UrllibTransport:
    def __init__(
        ...
        self.timeout_s = timeout_s
        self.internal_token = sha256_text(
            internal_token,
            "desktop executor internal token",
        )
    ...
    def request(
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "X-Llama-Warm-Tier-Internal": self.internal_token,
            },
        )
        try:
...
def read_warm_tier_internal_token_from_env() -> str:
    ...
    require(
        ...
        "desktop executor internal token file",
    )
    before = path.lstat()
    require(
        ...
        and before.st_mode & stat.S_IRUSR != 0
        and before.st_size == 65,
        "desktop executor internal token metadata",
    )
    descriptor = os.open(
        path,
    ...
    try:
        ...
        require(
            ...
            "desktop executor internal token changed before open",
        )
        pieces = []
        remaining = 65
        while remaining:
            piece = os.read(descriptor, remaining)
            require(piece, "desktop executor internal token truncated")
            pieces.append(piece)
            remaining -= len(piece)
        require(
            os.read(descriptor, 1) == b"",
            "desktop executor internal token too large",
        )
        ...
        require(
            ...
            "desktop executor internal token changed while reading",
        )
    finally:
        os.close(descriptor)
    raw = b"".join(pieces)
    require(
        raw.endswith(b"\n")
        and len(raw) == 65,
        "desktop executor internal token encoding",
    )
    try:
        token = raw[:-1].decode("ascii")
    except UnicodeDecodeError as error:
        raise GatewayError(
            "desktop executor internal token encoding") from error
    return sha256_text(token, "desktop executor internal token")
