#!/usr/bin/env python3
"""S9-V0-R1 deterministic offline residency simulator (bundle version 3).

Isolated research tool -- NOT integrated into llama-server. Integer-microsecond time,
stable total event order. The frozen pre-R1 simulator lives at
golden/v0r_historical/residency_sim_v0r.py; this file closes the fail-open mechanics an
independent adversarial pass found in V0-R:

  - readiness is backend/generation-qualified: an HTP-prepared residency is never
    dispatched to GPU (or the reverse);
  - stale_epoch makes the affected device DRAINING and its residency non-dispatchable;
  - a stale pipeline stage rolls back EXACTLY ONCE, releases its contention domain, and
    wakes the bounded prefetch queue;
  - the full epoch stack (boot/generation/route/state) is rechecked after D2H before the
    output is accepted;
  - mutable state is stored per request/session StateLease, with reset DEFERRED while
    the session is pinned;
  - HTP/GPU lane admission is bounded (overflow -> explicit rejected terminal);
  - event processing STOPS at the horizon (no unbounded polling);
  - a partial failure carries an explicit verified offset, so failures at different
    offsets produce different retry bytes and completion times;
  - link_drop has an explicit resumable (bulk) or terminal fail-closed (activation/
    compute/d2h) outcome;
  - server relief is counted ONLY after a valid successful phone completion;
  - measured interference is REJECTED as unsupported (it cannot yet be modeled
    symmetrically) rather than emitting misleading timing.

Every request still reaches exactly one terminal outcome and the run asserts
completed_phone + completed_server + fallback + rejected + timed_out == arrivals.
"""
import hashlib
import heapq
import json
import os
import sys

HERE = os.path.dirname(os.path.abspath(__file__))
US = 1000000

PR_FAULT, PR_ARRIVAL, PR_ACT, PR_STAGE, PR_LANE, PR_EXEC, PR_D2H, PR_EVICT, PR_WAIT = range(9)


def canonical(obj):
    return json.dumps(obj, sort_keys=True, separators=(",", ":"), ensure_ascii=True)


def sha_text(text):
    return "sha256:" + hashlib.sha256(text.encode("utf-8")).hexdigest()


def sha_bytes(b):
    return "sha256:" + hashlib.sha256(b).hexdigest()


def xfer_us(nbytes, goodput):
    return (nbytes * US + goodput - 1) // goodput


def rate_us(nbytes, rate):
    return (nbytes * US + rate - 1) // rate


def pctl(vals, qn, qd):
    if not vals:
        return 0
    n = len(vals)
    r = max(1, min(n, (qn * n + qd - 1) // qd))
    return vals[r - 1]


class FrameSequencer:
    """TransportFrame reader gate: rejects duplicate, reordered, and stale-epoch frames
    fail-closed; idempotency keys make a retried mutating frame a no-op."""
    def __init__(self):
        self.next_seq = {}
        self.applied = set()
        self.boot_epoch = {}
        self.residency_epoch = {}

    def accept(self, frame):
        conn = frame.get("conn", "c0")
        key = (conn, frame["channel"])
        exp = self.next_seq.get(key, frame["seq"])
        be = self.boot_epoch.get(conn)
        if be is not None and frame["boot_epoch"] < be:
            return False, "stale_epoch"
        re = self.residency_epoch.get(conn)
        if re is not None and frame.get("residency_epoch") is not None and frame["residency_epoch"] < re:
            return False, "stale_epoch"
        if frame["seq"] < exp:
            return False, "duplicate_message"
        if frame["seq"] > exp:
            return False, "reordered_message"
        if frame["idempotency_key"] in self.applied:
            return False, "duplicate_message"
        self.next_seq[key] = frame["seq"] + 1
        self.applied.add(frame["idempotency_key"])
        self.boot_epoch[conn] = max(be or 0, frame["boot_epoch"])
        if frame.get("residency_epoch") is not None:
            self.residency_epoch[conn] = max(re or 0, frame["residency_epoch"])
        return True, "ok"


class Ledger:
    def __init__(self, lpddr_total, ufs_total):
        self.total = lpddr_total
        self.canonical = self.derived = self.scratch = self.activations = self.state = 0
        self.ufs_total = ufs_total
        self.ufs_used = 0

    @property
    def free(self):
        return self.total - (self.canonical + self.derived + self.scratch + self.activations + self.state)

    def check(self):
        used = self.canonical + self.derived + self.scratch + self.activations + self.state
        assert 0 <= used <= self.total, f"lpddr overflow used={used} total={self.total}"
        assert self.free == self.total - used
        for v in (self.canonical, self.derived, self.scratch, self.activations, self.state):
            assert v >= 0
        assert 0 <= self.ufs_used <= self.ufs_total, f"ufs overflow {self.ufs_used}/{self.ufs_total}"

    def can_fit_lpddr(self, n):
        return self.free >= n

    def can_fit_ufs(self, n):
        return self.ufs_total - self.ufs_used >= n


class ResEntry:
    def __init__(self, ws_id, model):
        self.ws_id = ws_id
        self.model = model
        self.state = "ABSENT"
        self.backend = None
        self.ready_at = None
        self.generation = 0
        self.pipeline_ver = 0
        self.res_ufs = self.res_canonical = self.res_derived = self.res_scratch = 0
        self.pinned = 0
        self.last_use = -1
        self.uses = 0
        self.resumed = 0
        self.evict_after_unpin = False
        self.stale_cleaned = False
        self.reset_pending = False
        self.sticky_sessions = {}      # session_id -> reserved bytes (per-session mutable state)


class MeasuredInterferenceUnsupported(Exception):
    pass


class Sim:
    def __init__(self, cfg, goodput, policy):
        # R1: measured interference cannot yet be modeled symmetrically -> reject it.
        if cfg["interference"]["measured"]:
            raise MeasuredInterferenceUnsupported(
                "measured interference is unsupported in R1; would emit misleading timing")
        self.cfg = cfg
        self.policy = policy
        self.goodput = goodput
        self.horizon = cfg["horizon_us"]
        self.t = 0
        self.seq = 0
        self.heap = []
        self.phones = {p["device_id"]: p for p in cfg["phones"]}
        self.pdev = sorted(self.phones)
        self.rtt = {d: 500 for d in self.phones}
        self.ledger = {d: Ledger(p["lpddr_total_bytes"], p["ufs_total_bytes"]) for d, p in self.phones.items()}
        self.htp_free = {d: p["htp_lane_slots"] for d, p in self.phones.items()}
        self.gpu_free = {d: p["gpu_lane_slots"] for d, p in self.phones.items()}
        self.lane_q = {(d, b): [] for d in self.phones for b in ("htp", "gpu")}
        self.thermal_ok = {d: p["thermal_eligible"] for d, p in self.phones.items()}
        self.boot_epoch = {d: p["boot_epoch"] for d, p in self.phones.items()}
        self.route_epoch = {d: 0 for d in self.phones}
        self.state_epoch = {d: 0 for d in self.phones}
        self.draining = {d: False for d in self.phones}
        self.domain_of = {d: p["contention_domain"] for d, p in self.phones.items()}
        self.domains = sorted(set(self.domain_of.values()))
        self.domain_bulk = {dom: None for dom in self.domains}
        self.domain_link_busy = {dom: 0 for dom in self.domains}
        self.prefetch_q = {dom: [] for dom in self.domains}
        self.pf_depth = cfg["queues"]["prefetch_depth"]
        self.sv_depth = cfg["queues"]["server_depth"]
        self.lane_depth = cfg["queues"]["lane_depth"]
        self.models = {m["model_id"]: m for m in cfg["models"]}
        self.ws_model = {m["weight_set_id"]: m for m in cfg["models"]}
        self.res = {}
        self.server_gpu_free = cfg["server"]["gpu_lane_slots"]
        self.server_gpu_total = cfg["server"]["gpu_lane_slots"]
        self.server_hbm_free = cfg["server"]["hbm_credit_bytes"]
        self.server_hbm_total = cfg["server"]["hbm_credit_bytes"]
        self.server_q = []
        self.server_res = {}
        self.interf = cfg["interference"]
        # faults: single-shot kinds keyed (kind,target)->(at_us,phase); partial_transfer is a LIST
        self.faults = {}
        self.partials = {}
        for f in cfg.get("failure_schedule", []):
            if f["kind"] == "partial_transfer":
                self.partials.setdefault(f["target"], []).append((f["at_us"], f.get("verified_offset", 0)))
            else:
                self.faults.setdefault((f["kind"], f["target"]), (f["at_us"], f.get("phase")))
        for ws in self.partials:
            self.partials[ws].sort()
        self.partial_ptr = {ws: 0 for ws in self.partials}
        self.pp = cfg["policy_params"]
        self.predict = {}
        self.static_dev = {}
        self.terminal = {}
        self.req_res = {}
        self.session_inflight = set()
        self.req_index = {r["request_id"]: r for r in cfg["workload"]["requests"]}
        self.m = dict(dispatched_to_phone=0, weight_misses=0, server_gpu_us_used=0,
                      server_gpu_us_freed=0, server_hbm_bytes_freed=0, transfer_bytes=0,
                      retry_bytes=0, d2h_bytes=0, ufs_bytes=0, prepare_us_total=0, evictions=0,
                      deadline_hits=0, deadline_misses=0, objective_relief_us_total=0,
                      objective_cost_us_total=0, waited_for_weights=0, stale_pipeline_drops=0,
                      stale_completion_rejects=0, prefetch_drops=0, oversub_rollbacks=0,
                      lane_rejects=0, link_drop_terminal=0, link_drop_resumed=0,
                      reset_deferred=0, post_d2h_epoch_rejects=0, domain_releases_on_stale=0)
        self.outc = dict(completed_phone=0, completed_server=0, fallback=0, rejected=0, timed_out=0)
        self.dispatched_by_device = {d: 0 for d in self.phones}
        self.completions = []
        self.preemptions = 0
        self.quarantined = set()
        self.resumed_sets = set()
        self.live_state_evictions = 0
        self.dispatched_from_on_disk = 0

    # ---- heap ----
    def push(self, t, pr, kind, payload):
        self.seq += 1
        heapq.heappush(self.heap, (t, pr, self.seq, kind, payload))

    def fault_active(self, kind, target):
        v = self.faults.get((kind, target))
        return v is not None and v[0] <= self.t

    def resolve(self, dev, ws_id):
        key = (dev, ws_id)
        if key not in self.res:
            self.res[key] = ResEntry(ws_id, self.ws_model[ws_id])
        return self.res[key]

    def is_ready(self, dev, ws_id, back=None):
        r = self.res.get((dev, ws_id))
        if r is None or r.state not in ("READY_HTP", "READY_GPU"):
            return False
        return back is None or r.backend == back      # backend-qualified readiness

    def session_of(self, req):
        return req.get("session_id", req["request_id"])

    # ---- terminal bookkeeping ----
    def finish(self, req, outcome):
        rid = req["request_id"]
        if rid in self.terminal:
            return
        if self.t > self.horizon and outcome in ("completed_phone", "completed_server", "fallback"):
            outcome = "timed_out"
        self.terminal[rid] = outcome
        self.outc[outcome] += 1
        if outcome in ("completed_phone", "completed_server", "fallback"):
            self.completions.append(self.t - req["arrival_us"])
            if req["deadline_us"] is not None:
                if self.t <= req["deadline_us"]:
                    self.m["deadline_hits"] += 1
                else:
                    self.m["deadline_misses"] += 1

    # ---- link (per contention domain) ----
    def _link_send(self, dev, nbytes, is_result):
        dom = self.domain_of[dev]
        dur = xfer_us(nbytes, self.goodput) + self.rtt[dev]
        start = max(self.t, self.domain_link_busy[dom])
        finish = start + dur
        self.domain_link_busy[dom] = finish
        if is_result:
            self.m["d2h_bytes"] += nbytes
        bulk = self.domain_bulk[dom]
        if bulk is not None:
            self.preemptions += 1
            r = self.resolve(bulk["dev"], bulk["ws_id"])
            r.pipeline_ver += 1
            bulk["shift"] = bulk.get("shift", 0) + dur
            self.push(bulk["done_time"] + bulk["shift"], PR_STAGE, "res_stage",
                      {"dev": bulk["dev"], "ws_id": bulk["ws_id"], "stage": "transfer_done",
                       "ver": r.pipeline_ver, "gen": r.generation, "boot": self.boot_epoch[bulk["dev"]]})
        return finish

    def rollback_pipeline(self, dev, r):
        L = self.ledger[dev]
        L.canonical -= r.res_canonical
        L.derived -= r.res_derived
        L.scratch -= r.res_scratch
        L.ufs_used -= r.res_ufs
        r.res_canonical = r.res_derived = r.res_scratch = r.res_ufs = 0
        L.check()

    def free_residency(self, dev, ws, r):
        if r.pinned > 0 or r.sticky_sessions:
            self.live_state_evictions += 1
        L = self.ledger[dev]
        L.canonical -= r.res_canonical
        L.derived -= r.res_derived
        L.scratch -= r.res_scratch
        L.ufs_used -= r.res_ufs
        r.res_canonical = r.res_derived = r.res_scratch = r.res_ufs = 0
        r.state = "ABSENT"
        r.ready_at = None
        r.evict_after_unpin = False
        L.check()

    # ---- prefetch ----
    def start_prefetch(self, dev, ws_id):
        r = self.resolve(dev, ws_id)
        if r.state != "ABSENT" or ws_id in self.quarantined:
            return False
        if self.draining[dev] or self.fault_active("unknown_profile", dev):
            return False
        dom = self.domain_of[dev]
        if self.domain_bulk[dom] is not None:
            if len(self.prefetch_q[dom]) < self.pf_depth:
                if (dev, ws_id) not in self.prefetch_q[dom]:
                    self.prefetch_q[dom].append((dev, ws_id))
            else:
                self.m["prefetch_drops"] += 1
            return False
        model = self.ws_model[ws_id]
        w = model["canonical_bytes"]
        if not self.ledger[dev].can_fit_ufs(w) and not self.evict_for_ufs(dev, w):
            return False
        self.ledger[dev].ufs_used += w
        r.res_ufs = w
        r.state = "RECEIVING"
        r.stale_cleaned = False
        tdur = xfer_us(w, self.goodput)
        self.m["transfer_bytes"] += w
        start = max(self.t, self.domain_link_busy[dom])
        done = start + tdur + self.rtt[dev]
        r.pipeline_ver += 1
        self.domain_bulk[dom] = {"dev": dev, "ws_id": ws_id, "done_time": done, "ver": r.pipeline_ver}
        self.push(done, PR_STAGE, "res_stage", {"dev": dev, "ws_id": ws_id, "stage": "transfer_done",
                  "ver": r.pipeline_ver, "gen": r.generation, "boot": self.boot_epoch[dev]})
        return True

    def link_free(self, dom, owner):
        bulk = self.domain_bulk[dom]
        if bulk is None or (bulk["dev"], bulk["ws_id"]) != owner:
            return False
        self.domain_bulk[dom] = None
        while self.prefetch_q[dom]:
            dev, ws = self.prefetch_q[dom].pop(0)
            if self.res.get((dev, ws)) and self.res[(dev, ws)].state == "ABSENT":
                if self.start_prefetch(dev, ws):
                    break
        return True

    def clean_stale_pipeline(self, dev, ws_id, r):
        """Roll back a stale in-flight pipeline exactly once, release its domain link, and
        wake the bounded prefetch queue."""
        if r.stale_cleaned:
            return
        r.stale_cleaned = True
        self.m["stale_pipeline_drops"] += 1
        self.rollback_pipeline(dev, r)
        if r.state not in ("READY_HTP", "READY_GPU"):
            r.state = "ABSENT"
        dom = self.domain_of[dev]
        if self.link_free(dom, (dev, ws_id)):
            self.m["domain_releases_on_stale"] += 1

    def stage(self, p):
        dev, ws_id, st = p["dev"], p["ws_id"], p["stage"]
        r = self.resolve(dev, ws_id)
        if p["gen"] != r.generation or p["boot"] != self.boot_epoch[dev]:
            self.clean_stale_pipeline(dev, ws_id, r)     # genuine epoch invalidation: roll back + release + wake
            return
        if p["ver"] != r.pipeline_ver:
            return                                       # superseded by a newer event for the same residency
                                                          # (activation preemption / link_drop or partial resume):
                                                          # the live event carries the reservation forward -> no-op
        model = self.ws_model[ws_id]
        phone = self.phones[dev]
        dom = self.domain_of[dev]
        if st == "transfer_done":
            plist = self.partials.get(ws_id)
            if plist is not None and self.partial_ptr[ws_id] < len(plist) and plist[self.partial_ptr[ws_id]][0] <= self.t:
                _, voff = plist[self.partial_ptr[ws_id]]
                self.partial_ptr[ws_id] += 1
                r.resumed += 1
                self.resumed_sets.add(ws_id)
                remaining = max(1, model["canonical_bytes"] - voff)   # re-send from the verified offset
                self.m["retry_bytes"] += remaining
                self.m["transfer_bytes"] += remaining
                rdur = xfer_us(remaining, self.goodput)
                r.pipeline_ver += 1
                start = max(self.t, self.domain_link_busy[dom])
                self.domain_bulk[dom] = {"dev": dev, "ws_id": ws_id,
                                         "done_time": start + rdur + self.rtt[dev], "ver": r.pipeline_ver}
                self.push(start + rdur + self.rtt[dev], PR_STAGE, "res_stage",
                          {"dev": dev, "ws_id": ws_id, "stage": "transfer_done", "ver": r.pipeline_ver,
                           "gen": r.generation, "boot": self.boot_epoch[dev]})
                return
            assert self.link_free(dom, (dev, ws_id)), "transfer completion does not own contention domain"
            self.m["ufs_bytes"] += model["canonical_bytes"]
            r.pipeline_ver += 1
            self.push(self.t + rate_us(model["canonical_bytes"], phone["ufs_write_bytes_per_s"]), PR_STAGE,
                      "res_stage", {"dev": dev, "ws_id": ws_id, "stage": "ufs_done", "ver": r.pipeline_ver,
                                    "gen": r.generation, "boot": self.boot_epoch[dev]})
        elif st == "ufs_done":
            r.state = "VERIFYING"
            if self.fault_active("hash_mismatch", ws_id):
                r.state = "QUARANTINED"
                self.quarantined.add(ws_id)
                self.rollback_pipeline(dev, r)
                return
            r.pipeline_ver += 1
            self.push(self.t + rate_us(model["canonical_bytes"], phone["verify_bytes_per_s"]), PR_STAGE,
                      "res_stage", {"dev": dev, "ws_id": ws_id, "stage": "verify_done", "ver": r.pipeline_ver,
                                    "gen": r.generation, "boot": self.boot_epoch[dev]})
        elif st == "verify_done":
            r.state = "VERIFIED_ON_DISK"
            w = model["canonical_bytes"]
            if not self.ledger[dev].can_fit_lpddr(w) and not self.evict_for_lpddr(dev, w):
                self.m["oversub_rollbacks"] += 1
                self.rollback_pipeline(dev, r)
                r.state = "ABSENT"
                return
            self.ledger[dev].canonical += w
            r.res_canonical = w
            self.ledger[dev].check()
            r.state = "MATERIALIZING"
            r.pipeline_ver += 1
            self.push(self.t + rate_us(w, phone["materialize_bytes_per_s"]), PR_STAGE, "res_stage",
                      {"dev": dev, "ws_id": ws_id, "stage": "materialize_done", "ver": r.pipeline_ver,
                       "gen": r.generation, "boot": self.boot_epoch[dev]})
        elif st == "materialize_done":
            r.state = "LPDDR_READY"
            back = model["eligible_backends"][0]
            r.backend = back
            d = model["derived_bytes_htp"] if back == "htp" else model["derived_bytes_gpu"]
            if d > 0:
                if not self.ledger[dev].can_fit_lpddr(d) and not self.evict_for_lpddr(dev, d):
                    self.m["oversub_rollbacks"] += 1
                    self.rollback_pipeline(dev, r)
                    r.state = "ABSENT"
                    return
                self.ledger[dev].derived += d
                r.res_derived = d
                self.ledger[dev].check()
            pr_rate = phone["prepare_htp_bytes_per_s"] if back == "htp" else phone["prepare_gpu_bytes_per_s"]
            pdur = rate_us(d, pr_rate) if d > 0 else 0
            self.m["prepare_us_total"] += pdur
            r.state = "PREPARING_HTP" if back == "htp" else "PREPARING_GPU"
            r.pipeline_ver += 1
            self.push(self.t + pdur, PR_STAGE, "res_stage", {"dev": dev, "ws_id": ws_id,
                      "stage": "prepare_done", "ver": r.pipeline_ver, "gen": r.generation, "boot": self.boot_epoch[dev]})
        elif st == "prepare_done":
            r.state = "WARMING"
            r.pipeline_ver += 1
            self.push(self.t + phone["warmup_us"], PR_STAGE, "res_stage", {"dev": dev, "ws_id": ws_id,
                      "stage": "warm_done", "ver": r.pipeline_ver, "gen": r.generation, "boot": self.boot_epoch[dev]})
        elif st == "warm_done":
            s = model["scratch_bytes"]
            if not self.ledger[dev].can_fit_lpddr(s) and not self.evict_for_lpddr(dev, s):
                self.m["oversub_rollbacks"] += 1
                self.rollback_pipeline(dev, r)
                r.state = "ABSENT"
                return
            self.ledger[dev].scratch += s
            r.res_scratch = s
            self.ledger[dev].check()
            r.state = "READY_HTP" if r.backend == "htp" else "READY_GPU"
            r.ready_at = self.t

    # ---- eviction ----
    def victims(self, dev):
        return [(dd, ws, r) for (dd, ws), r in sorted(self.res.items())
                if dd == dev and r.state in ("READY_HTP", "READY_GPU") and r.pinned == 0 and not r.sticky_sessions]

    def pick_victim(self, cands):
        ttl = self.pp["ttl_us"]
        if self.policy == "clairvoyant":
            cands.sort(key=lambda c: (-self.next_use(c[1]), c[1]))
        elif self.policy == "lfu_ttl":
            cands.sort(key=lambda c: (0 if (self.t - c[2].last_use) > ttl else 1, c[2].uses, c[2].last_use, c[1]))
        else:
            cands.sort(key=lambda c: (c[2].last_use, c[1]))
        return cands[0]

    def evict_for_lpddr(self, dev, need):
        min_hold = self.pp["min_hold_us"]
        while not self.ledger[dev].can_fit_lpddr(need):
            cands = self.victims(dev)
            aged = [c for c in cands if c[2].ready_at is not None and (self.t - c[2].ready_at) >= min_hold]
            pool = aged if aged else cands
            if not pool:
                return False
            dd, ws, r = self.pick_victim(pool)
            self.free_residency(dd, ws, r)
            self.m["evictions"] += 1
        return True

    def evict_for_ufs(self, dev, need):
        while not self.ledger[dev].can_fit_ufs(need):
            cands = self.victims(dev)
            if not cands:
                return False
            dd, ws, r = self.pick_victim(cands)
            self.free_residency(dd, ws, r)
            self.m["evictions"] += 1
        return True

    def next_use(self, ws_id):
        nxt = None
        for req in self.cfg["workload"]["requests"]:
            if req["weight_set_id"] == ws_id and req["arrival_us"] > self.t:
                if nxt is None or req["arrival_us"] < nxt:
                    nxt = req["arrival_us"]
        return nxt if nxt is not None else (1 << 62)

    # ---- server ----
    def _start_server(self, req, gpu_us, hbm, bucket):
        assert self.server_gpu_free >= 1 and self.server_hbm_free >= hbm
        self.server_gpu_free -= 1
        self.server_hbm_free -= hbm
        self.m["server_gpu_us_used"] += gpu_us
        self.server_res[req["request_id"]] = {
            "active": True, "gpu_us": gpu_us, "hbm": hbm, "start_us": self.t,
        }
        self.push(self.t + gpu_us, PR_EXEC, "server_done", {"req": req, "hbm": hbm, "bucket": bucket})

    def route_server(self, req, bucket):
        cls = req["service_class"]
        hbm = self.cfg["server"]["per_class_hbm_bytes"].get(cls, 0)
        if hbm > self.server_hbm_total:
            self.finish(req, "rejected")
            return
        gpu_us = self.cfg["server"]["per_class_gpu_us"].get(cls, 1000)
        if self.server_gpu_free >= 1 and self.server_hbm_free >= hbm:
            self._start_server(req, gpu_us, hbm, bucket)
        elif len(self.server_q) < self.sv_depth:
            self.server_q.append((req, gpu_us, hbm, bucket))
            self.server_res[req["request_id"]] = {
                "active": False, "gpu_us": gpu_us, "hbm": hbm, "start_us": None,
            }
        else:
            self.finish(req, "rejected")

    def server_done(self, p):
        self.server_res.pop(p["req"]["request_id"], None)
        self.server_gpu_free += 1
        self.server_hbm_free += p["hbm"]
        self.finish(p["req"], p["bucket"])
        if self.server_q:
            req, gpu_us, hbm, bucket = self.server_q[0]
            if self.server_gpu_free >= 1 and self.server_hbm_free >= hbm:
                self.server_q.pop(0)
                self._start_server(req, gpu_us, hbm, bucket)

    def _release_server_at_horizon(self, req):
        rid = req["request_id"]
        sr = self.server_res.pop(rid, None)
        if sr is None:
            return
        if sr["active"]:
            self.server_gpu_free += 1
            self.server_hbm_free += sr["hbm"]
            executed = max(0, min(sr["gpu_us"], self.horizon - sr["start_us"]))
            self.m["server_gpu_us_used"] -= sr["gpu_us"] - executed
        else:
            self.server_q[:] = [x for x in self.server_q if x[0]["request_id"] != rid]

    # ---- eligibility ----
    def eligible(self, dev, back, req):
        model = self.models[req["model_id"]]
        if self.draining[dev]:
            return False, "draining"
        if self.fault_active("unknown_profile", dev):
            return False, "unknown_profile"
        if self.fault_active("unsupported_backend", req["weight_set_id"]):
            return False, "unsupported_backend"
        if back not in model["eligible_backends"]:
            return False, "unsupported_backend"
        if not self.thermal_ok[dev]:
            return False, "thermal"
        d = model["derived_bytes_htp"] if back == "htp" else model["derived_bytes_gpu"]
        if model["canonical_bytes"] + d + model["scratch_bytes"] > self.ledger[dev].total:
            return False, "insufficient_ram"
        return True, "ok"

    def candidates(self, req):
        model = self.models[req["model_id"]]
        return [(dev, back) for dev in self.pdev for back in model["eligible_backends"]
                if self.eligible(dev, back, req)[0]]

    def cold_cost_us(self, dev, back, model):
        phone = self.phones[dev]
        w = model["canonical_bytes"]
        d = model["derived_bytes_htp"] if back == "htp" else model["derived_bytes_gpu"]
        pr_rate = phone["prepare_htp_bytes_per_s"] if back == "htp" else phone["prepare_gpu_bytes_per_s"]
        return (xfer_us(w, self.goodput) + rate_us(w, phone["ufs_write_bytes_per_s"]) +
                rate_us(w, phone["verify_bytes_per_s"]) + rate_us(w, phone["materialize_bytes_per_s"]) +
                (rate_us(d, pr_rate) if d > 0 else 0) + phone["warmup_us"])

    def objective_key(self, dev, back, req):
        model = self.models[req["model_id"]]
        relief = self.cfg["server"]["per_class_gpu_us"].get(req["service_class"], 0)
        cold = self.cold_cost_us(dev, back, model)
        reuse = self.predict.get(req["weight_set_id"], 0)
        ready = self.is_ready(dev, req["weight_set_id"], back)
        need = 0 if ready else (model["canonical_bytes"] +
                                (model["derived_bytes_htp"] if back == "htp" else model["derived_bytes_gpu"]))
        evict_loss = max(0, need - self.ledger[dev].free)
        slo = 1
        if req["deadline_us"] is not None:
            eta = self.t + (0 if ready else cold) + model["phone_compute_us"] + xfer_us(req["output_bytes"], self.goodput)
            slo = 1 if eta <= req["deadline_us"] else 0
        return (slo, relief, -cold, reuse, -evict_loss, -self.pdev.index(dev))

    # ---- dispatch ----
    def dispatch_phone(self, dev, back, req):
        ws = req["weight_set_id"]
        if not self.is_ready(dev, ws, back):          # backend-qualified: refuse wrong-backend residency
            self.dispatched_from_on_disk += 1
            return False
        if self.draining[dev]:
            return False
        act, st = req["input_bytes"], req["state_bytes"]
        model = self.models[req["model_id"]]
        sticky = model["state_policy"] in ("sticky", "rebuildable")
        r = self.resolve(dev, ws)
        sess = self.session_of(req)
        session_key = (dev, ws, sess)
        if sticky and session_key in self.session_inflight:
            return False
        new_state = 0 if (sticky and sess in r.sticky_sessions) else st
        if not self.ledger[dev].can_fit_lpddr(act + new_state):
            return False
        self.ledger[dev].activations += act
        self.ledger[dev].state += new_state
        self.ledger[dev].check()
        r.pinned += 1
        if sticky:
            self.session_inflight.add(session_key)
        r.last_use = self.t
        r.uses += 1
        self.dispatched_by_device[dev] += 1
        self.req_res[req["request_id"]] = {"dev": dev, "ws": ws, "act": act, "st": new_state,
                                           "back": back, "sticky": sticky, "session": sess,
                                           "session_key": session_key if sticky else None,
                                           "phase": "activation", "holds_lane": False, "released": False}
        in_done = self._link_send(dev, act, is_result=False)
        self.push(in_done, PR_LANE, "lane_arrive", {"dev": dev, "back": back, "req": req,
                  "boot": self.boot_epoch[dev], "gen": r.generation, "route": self.route_epoch[dev],
                  "state": self.state_epoch[dev]})
        return True

    def _after_unpin(self, dev, ws, r):
        self._maybe_deferred_reset(dev, r)
        if r.pinned == 0 and not r.sticky_sessions and r.evict_after_unpin:
            self.free_residency(dev, ws, r)
            self.m["evictions"] += 1

    def _release_phone_reservation(self, req):
        rid = req["request_id"]
        rr = self.req_res.get(rid, {})
        if not rr or rr.get("released"):
            return
        dev = rr.get("dev")
        if dev is not None:
            self.lane_q[(dev, rr["back"])] = [
                p for p in self.lane_q[(dev, rr["back"])] if p["req"]["request_id"] != rid]
            self.ledger[dev].activations -= rr.get("act", 0)
            self.ledger[dev].state -= rr.get("st", 0)
            self.ledger[dev].check()
            r = self.resolve(dev, rr["ws"])
            assert r.pinned > 0
            r.pinned -= 1
            if rr.get("holds_lane"):
                rr["holds_lane"] = False
                self._release_lane(dev, rr["back"])
            if rr.get("session_key") is not None:
                self.session_inflight.discard(rr["session_key"])
            rr["released"] = True
            rr["phase"] = "terminal"
            self._after_unpin(dev, rr["ws"], r)

    def _reject_inflight(self, req, metric):
        rid = req["request_id"]
        if rid in self.terminal:
            return
        self._release_phone_reservation(req)
        self.m[metric] += 1
        self.finish(req, "rejected")

    def lane_arrive(self, p):
        dev, back, req = p["dev"], p["back"], p["req"]
        if req["request_id"] in self.terminal:
            return
        free = self.htp_free if back == "htp" else self.gpu_free
        comp = self.models[req["model_id"]]["phone_compute_us"]
        if free[dev] >= 1:
            free[dev] -= 1
            self.req_res[req["request_id"]]["phase"] = "compute"
            self.req_res[req["request_id"]]["holds_lane"] = True
            self.push(self.t + comp, PR_EXEC, "exec_done", {**p, "comp": comp})
        elif len(self.lane_q[(dev, back)]) < self.lane_depth:
            self.lane_q[(dev, back)].append({**p, "comp": comp})
        else:
            self._reject_inflight(req, "lane_rejects")    # bounded lane admission

    def exec_done(self, p):
        dev, back, req = p["dev"], p["back"], p["req"]
        if req["request_id"] in self.terminal:
            # the request was terminalized out-of-band (e.g. compute-phase link_drop reject),
            # which already released its lane; release again ONLY if this request still holds it.
            rr = self.req_res.get(req["request_id"], {})
            if rr.get("holds_lane"):
                rr["holds_lane"] = False
                self._release_lane(dev, back)
            return
        r = self.resolve(dev, req["weight_set_id"])
        stale = (p["boot"] != self.boot_epoch[dev] or p["gen"] != r.generation or
                 p["route"] != self.route_epoch[dev] or p["state"] != self.state_epoch[dev])
        if stale:
            self.req_res[req["request_id"]]["holds_lane"] = False
            self._release_lane(dev, back)
            self._reject_inflight(req, "stale_completion_rejects")
            return
        self.req_res[req["request_id"]]["phase"] = "d2h"
        self.req_res[req["request_id"]]["holds_lane"] = False
        out = self._link_send(dev, req["output_bytes"], is_result=True)
        self.push(out, PR_D2H, "d2h_done", p)
        self._release_lane(dev, back)

    def _release_lane(self, dev, back):
        free = self.htp_free if back == "htp" else self.gpu_free
        free[dev] += 1
        q = self.lane_q[(dev, back)]
        while q:
            nxt = q.pop(0)
            if nxt["req"]["request_id"] in self.terminal:
                continue
            free[dev] -= 1
            self.req_res[nxt["req"]["request_id"]]["phase"] = "compute"
            self.req_res[nxt["req"]["request_id"]]["holds_lane"] = True
            self.push(self.t + nxt["comp"], PR_EXEC, "exec_done", nxt)
            break

    def d2h_done(self, p):
        dev, req = p["dev"], p["req"]
        rid = req["request_id"]
        if rid in self.terminal:
            return
        r = self.resolve(dev, req["weight_set_id"])
        # R1: recheck the FULL epoch stack after D2H before accepting the output
        if (p["boot"] != self.boot_epoch[dev] or p["gen"] != r.generation or
                p["route"] != self.route_epoch[dev] or p["state"] != self.state_epoch[dev]):
            self._reject_inflight(req, "post_d2h_epoch_rejects")
            return
        rr = self.req_res.get(rid, {})
        self.ledger[dev].activations -= rr.get("act", 0)
        if rr.get("sticky"):
            if rr.get("st", 0) > 0:
                r.sticky_sessions[rr["session"]] = r.sticky_sessions.get(rr["session"], 0) + rr["st"]
        else:
            self.ledger[dev].state -= rr.get("st", 0)
        self.ledger[dev].check()
        assert r.pinned > 0
        r.pinned -= 1
        # relief is credited ONLY here, after a valid successful phone completion
        cls = req["service_class"]
        self.m["server_gpu_us_freed"] += self.cfg["server"]["per_class_gpu_us"].get(cls, 0)
        self.m["server_hbm_bytes_freed"] += self.cfg["server"]["per_class_hbm_bytes"].get(cls, 0)
        self.m["objective_relief_us_total"] += self.cfg["server"]["per_class_gpu_us"].get(cls, 0)
        if rr.get("session_key") is not None:
            self.session_inflight.discard(rr["session_key"])
        rr["released"] = True
        rr["phase"] = "terminal"
        self._after_unpin(dev, req["weight_set_id"], r)
        self.finish(req, "completed_phone")

    def _maybe_deferred_reset(self, dev, r):
        if r.reset_pending and r.pinned == 0:
            for _, b in list(r.sticky_sessions.items()):
                self.ledger[dev].state -= b
            r.sticky_sessions.clear()
            r.reset_pending = False
            self.ledger[dev].check()

    # ---- arrival policy ----
    def preplace(self):
        seen = []
        for req in self.cfg["workload"]["requests"]:
            if req["weight_set_id"] not in seen:
                seen.append(req["weight_set_id"])
        for i, ws in enumerate(seen):
            model = self.ws_model[ws]
            elig = [d for d in self.pdev if self.eligible(d, model["eligible_backends"][0], self._probe(ws))[0]]
            self.static_dev[ws] = elig[i % len(elig)] if elig else (self.pdev[0] if self.pdev else None)
        if self.policy in ("static_placement", "clairvoyant"):
            for ws in seen:
                dev = self.static_dev[ws] if self.policy == "static_placement" else self.pdev[0]
                if self.policy == "clairvoyant" and self.next_use(ws) >= self.pp["reuse_horizon_us"]:
                    continue
                if dev is not None:
                    self.start_prefetch(dev, ws)

    def _probe(self, ws):
        m = self.ws_model[ws]
        return {"model_id": m["model_id"], "weight_set_id": ws, "service_class": "decode",
                "input_bytes": 0, "output_bytes": 0, "state_bytes": 0, "deadline_us": None}

    def on_arrival(self, req):
        ws = req["weight_set_id"]
        self.predict[ws] = (self.predict[ws] * (1000 - self.pp["predictor_ewma_permille"]) +
                            1000 * self.pp["predictor_ewma_permille"]) // 1000 if ws in self.predict \
            else self.pp["predictor_ewma_permille"]
        if self.policy == "server_only":
            self.route_server(req, "completed_server")
            return
        cands = self.candidates(req)
        if not cands:
            self.route_server(req, "fallback")
            return
        if self.policy == "per_request_fetch":
            self._per_request_fetch(req, cands)
            return
        ready = [(d, b) for (d, b) in cands if self.is_ready(d, ws, b)]
        pick = self._select(req, cands, ready)
        if pick is not None and self.is_ready(pick[0], ws, pick[1]):
            if self.dispatch_phone(pick[0], pick[1], req):
                self.m["dispatched_to_phone"] += 1
                return
        self.m["weight_misses"] += 1
        self.route_server(req, "fallback")
        self._maybe_prefetch(req, cands, pick)

    def _select(self, req, cands, ready):
        ws = req["weight_set_id"]
        if self.policy == "static_placement":
            dev = self.static_dev.get(ws)
            return next(((d, b) for (d, b) in cands if d == dev), cands[0])
        if self.policy in ("lru", "lfu_ttl"):
            home = self.static_dev.get(ws)
            return next(((d, b) for (d, b) in ready if d == home),
                        ready[0] if ready else next(((d, b) for (d, b) in cands if d == home), cands[0]))
        if self.policy == "fastest_ready":
            pool = sorted(ready if ready else cands, key=lambda c: (self.models[req["model_id"]]["phone_compute_us"], c[0]))
            return pool[0]
        if self.policy == "clairvoyant":
            return sorted(ready if ready else cands, key=lambda c: (0 if self.is_ready(c[0], ws, c[1]) else 1, self.pdev.index(c[0])))[0]
        return max(ready if ready else cands, key=lambda c: self.objective_key(c[0], c[1], req))

    def _maybe_prefetch(self, req, cands, pick):
        ws = req["weight_set_id"]
        if self.policy == "static_placement":
            if self.static_dev.get(ws) is not None:
                self.start_prefetch(self.static_dev[ws], ws)
            return
        if self.policy == "relief_predictive":
            best = max(cands, key=lambda c: self.objective_key(c[0], c[1], req))
            if self.objective_key(best[0], best[1], req)[1] <= 0:
                return
            self.start_prefetch(best[0], ws)
            return
        if self.policy == "clairvoyant":
            if self.next_use(ws) >= self.pp["reuse_horizon_us"]:
                return
            self.start_prefetch(self.pdev[0], ws)
            return
        self.start_prefetch((pick[0] if pick else cands[0][0]), ws)

    def _per_request_fetch(self, req, cands):
        ws, dev, back = req["weight_set_id"], cands[0][0], cands[0][1]
        if self.is_ready(dev, ws, back):
            if not self.dispatch_phone(dev, back, req):
                self.route_server(req, "fallback")
            else:
                self.m["dispatched_to_phone"] += 1
            return
        self.start_prefetch(dev, ws)
        self.m["waited_for_weights"] += 1
        self.push(self.t + 1000, PR_WAIT, "wait", {"dev": dev, "back": back, "req": req})

    def wait(self, p):
        dev, back, req = p["dev"], p["back"], p["req"]
        ws = req["weight_set_id"]
        if req["request_id"] in self.terminal:
            return
        if self.is_ready(dev, ws, back):
            if self.dispatch_phone(dev, back, req):
                self.m["dispatched_to_phone"] += 1
            else:
                self.route_server(req, "fallback")
            return
        r = self.res.get((dev, ws))
        if r is not None and r.state == "QUARANTINED":
            self.route_server(req, "fallback")
            return
        if self.draining[dev]:
            self.route_server(req, "fallback")
            return
        if r is not None and r.state == "ABSENT":
            self.start_prefetch(dev, ws)
        self.push(self.t + 1000, PR_WAIT, "wait", p)   # bounded: the run loop stops at horizon

    # ---- faults ----
    def fault_event(self, kind, target, phase=None):
        if kind == "thermal_trip" and target in self.phones:
            self.thermal_ok[target] = False
        elif kind == "drain" and target in self.phones:
            self.draining[target] = True
        elif kind == "reset_state" and target in self.phones:
            for (dd, ws), r in sorted(self.res.items()):
                if dd != target or not r.sticky_sessions:
                    continue
                if r.pinned == 0:
                    for _, b in list(r.sticky_sessions.items()):
                        self.ledger[dd].state -= b
                    r.sticky_sessions.clear()
                    self.ledger[dd].check()
                else:
                    r.reset_pending = True                # deferred reset while pinned
                    self.m["reset_deferred"] += 1
        elif kind == "stale_epoch" and target in self.phones:
            self.boot_epoch[target] += 1
            self.route_epoch[target] += 1
            self.state_epoch[target] += 1
            self.draining[target] = True                  # non-dispatchable + DRAINING
            for (dd, ws), r in sorted(self.res.items()):
                if dd == target and r.state in ("READY_HTP", "READY_GPU"):
                    if r.pinned > 0 or r.sticky_sessions:
                        r.evict_after_unpin = True
                    else:
                        self.free_residency(dd, ws, r)
        elif kind == "link_drop" and target in self.phones:
            self._link_drop(target, phase)

    def _link_drop(self, dev, phase):
        dom = self.domain_of[dev]
        if phase == "bulk":
            bulk = self.domain_bulk[dom]
            if bulk is not None and bulk["dev"] == dev:
                r = self.resolve(bulk["dev"], bulk["ws_id"])
                model = self.ws_model[bulk["ws_id"]]
                self.m["retry_bytes"] += model["canonical_bytes"]     # resumable: re-fetch from 0
                self.m["transfer_bytes"] += model["canonical_bytes"]
                self.m["link_drop_resumed"] += 1
                r.resumed += 1
                self.resumed_sets.add(bulk["ws_id"])
                r.pipeline_ver += 1
                start = max(self.t, self.domain_link_busy[dom])
                rdur = xfer_us(model["canonical_bytes"], self.goodput)
                self.domain_bulk[dom] = {"dev": bulk["dev"], "ws_id": bulk["ws_id"],
                                         "done_time": start + rdur + self.rtt[bulk["dev"]], "ver": r.pipeline_ver}
                self.push(start + rdur + self.rtt[bulk["dev"]], PR_STAGE, "res_stage",
                          {"dev": bulk["dev"], "ws_id": bulk["ws_id"], "stage": "transfer_done",
                           "ver": r.pipeline_ver, "gen": r.generation, "boot": self.boot_epoch[bulk["dev"]]})
        else:  # activation / compute / d2h: terminal fail-closed for in-flight requests
            # SNAPSHOT the victims before mutating: rejecting one request releases its lane, which
            # promotes a queued request to 'compute' mid-loop; a live re-scan would fold that
            # freshly-promoted request into a compute-phase drop it never belonged to.
            victims = [rid for rid, rr in self.req_res.items()
                       if rr["dev"] == dev and rr.get("phase") == phase and rid not in self.terminal]
            for rid in victims:
                if rid not in self.terminal:
                    self._reject_inflight(self.req_index[rid], "link_drop_terminal")

    def _check_invariants(self):
        active = [(rid, rr) for rid, rr in self.req_res.items()
                  if not rr.get("released") and rid not in self.terminal]
        assert all(rr.get("released") for rid, rr in self.req_res.items() if rid in self.terminal)
        expected_sessions = {rr["session_key"] for _, rr in active if rr.get("session_key") is not None}
        assert self.session_inflight == expected_sessions, "session mutation ownership mismatch"
        for d in self.phones:
            L = self.ledger[d]
            L.check()
            htp_holders = sum(1 for _, rr in active if rr["dev"] == d and rr["back"] == "htp" and rr["holds_lane"])
            gpu_holders = sum(1 for _, rr in active if rr["dev"] == d and rr["back"] == "gpu" and rr["holds_lane"])
            assert self.htp_free[d] + htp_holders == self.phones[d]["htp_lane_slots"], "htp lane conservation"
            assert self.gpu_free[d] + gpu_holders == self.phones[d]["gpu_lane_slots"], "gpu lane conservation"
            assert L.activations == sum(rr["act"] for _, rr in active if rr["dev"] == d), "activation partition"
            committed_state = sum(sum(r.sticky_sessions.values()) for (dd, _), r in self.res.items() if dd == d)
            pending_state = sum(rr["st"] for _, rr in active if rr["dev"] == d)
            assert L.state == committed_state + pending_state, "state partition"
            assert L.canonical == sum(r.res_canonical for (dd, _), r in self.res.items() if dd == d)
            assert L.derived == sum(r.res_derived for (dd, _), r in self.res.items() if dd == d)
            assert L.scratch == sum(r.res_scratch for (dd, _), r in self.res.items() if dd == d)
            assert L.ufs_used == sum(r.res_ufs for (dd, _), r in self.res.items() if dd == d)
            for (dd, ws), r in self.res.items():
                if dd != d:
                    continue
                pins = sum(1 for _, rr in active if rr["dev"] == d and rr["ws"] == ws)
                assert r.pinned == pins, "residency pin partition"
                if r.state not in ("ABSENT", "QUARANTINED"):
                    assert r.res_ufs == self.ws_model[ws]["canonical_bytes"], "live residency lacks UFS reservation"
                assert not (self.draining[d] and r.evict_after_unpin and r.pinned == 0
                            and not r.sticky_sessions and r.state in ("READY_HTP", "READY_GPU")), \
                    "drained residency missed deferred eviction"
        active_server = [r for r in self.server_res.values() if r["active"]]
        assert self.server_gpu_free + len(active_server) == self.server_gpu_total, "server lane conservation"
        assert self.server_hbm_free + sum(r["hbm"] for r in active_server) == self.server_hbm_total, \
            "server HBM conservation"

    def run(self):
        for (kind, target), (at, phase) in sorted(self.faults.items()):
            self.push(at, PR_FAULT, "fault", {"kind": kind, "target": target, "phase": phase})
        self.preplace()
        self._check_invariants()
        for req in sorted(self.cfg["workload"]["requests"], key=lambda r: (r["arrival_us"], r["rank"])):
            self.push(req["arrival_us"], PR_ARRIVAL, "arrival", req)
        while self.heap:
            t, pr, seq, kind, payload = heapq.heappop(self.heap)
            if t > self.horizon:                          # STOP at the horizon (no unbounded polling)
                break
            self.t = t
            if kind == "arrival":
                self.on_arrival(payload)
            elif kind == "wait":
                self.wait(payload)
            elif kind == "res_stage":
                self.stage(payload)
            elif kind == "lane_arrive":
                self.lane_arrive(payload)
            elif kind == "exec_done":
                self.exec_done(payload)
            elif kind == "d2h_done":
                self.d2h_done(payload)
            elif kind == "server_done":
                self.server_done(payload)
            elif kind == "fault":
                self.fault_event(payload["kind"], payload["target"], payload.get("phase"))
            self._check_invariants()
        self.t = self.horizon
        for req in self.cfg["workload"]["requests"]:
            if req["request_id"] not in self.terminal:
                self._release_phone_reservation(req)
                self._release_server_at_horizon(req)
                self.finish(req, "timed_out")
        self._check_invariants()
        return self.result()

    def result(self):
        comps = sorted(self.completions)
        total = len(self.cfg["workload"]["requests"])
        assert sum(self.outc.values()) == total, f"terminal buckets {self.outc} != {total} arrivals"
        r = dict(self.m)
        r.update(self.outc)
        r["policy"] = self.policy
        r["requests"] = total
        r["is_upper_bound"] = (self.policy == "clairvoyant")
        r["is_diagnostic_losing"] = (self.policy == "per_request_fetch")
        r["dispatched_by_device"] = dict(sorted(self.dispatched_by_device.items()))
        r["completion_p50_us"] = pctl(comps, 1, 2)
        r["completion_p95_us"] = pctl(comps, 19, 20)
        keys = ["policy", "is_upper_bound", "is_diagnostic_losing", "requests",
                "completed_phone", "completed_server", "fallback", "rejected", "timed_out",
                "dispatched_to_phone", "dispatched_by_device", "weight_misses",
                "server_gpu_us_used", "server_gpu_us_freed", "server_hbm_bytes_freed",
                "transfer_bytes", "retry_bytes", "d2h_bytes", "ufs_bytes", "prepare_us_total",
                "evictions", "stale_pipeline_drops", "stale_completion_rejects", "prefetch_drops",
                "oversub_rollbacks", "lane_rejects", "link_drop_terminal", "link_drop_resumed",
                "reset_deferred", "post_d2h_epoch_rejects", "domain_releases_on_stale",
                "deadline_hits", "deadline_misses", "completion_p50_us", "completion_p95_us",
                "objective_relief_us_total", "objective_cost_us_total", "waited_for_weights"]
        return {k: r[k] for k in keys}


def run_sweep(cfg):
    results = []
    for g in cfg["link_goodput_sweep_bytes_per_s"]:
        row = {"goodput_bytes_per_s": g, "baselines": []}
        for pol in cfg["baselines"]:
            row["baselines"].append(Sim(cfg, g, pol).run())
        results.append(row)
    return results


def code_version():
    return sha_bytes(open(__file__, "rb").read())


def build_manifest(cfg, cfg_path, results):
    return {
        "schema_version": 3, "run_id": "run-" + cfg["config_id"], "kind": "residency_sim",
        "fixtures_only": True, "config_id": cfg["config_id"], "config_hash": sha_text(canonical(cfg)),
        "code_version": code_version(), "seed": cfg["seed"],
        "inputs": [{"role": "sim_config", "path": os.path.relpath(cfg_path, HERE),
                    "sha256": sha_bytes(open(cfg_path, "rb").read())}],
        "goodput_results": results, "deterministic_replay_sha256": sha_text(canonical(results)),
        "gate_results": {},
    }


def main(argv):
    if not argv:
        print("usage: residency_sim.py CONFIG.json [--out MANIFEST.json]")
        return 2
    cfg = json.load(open(argv[0]))
    manifest = build_manifest(cfg, argv[0], run_sweep(cfg))
    if "--out" in argv:
        open(argv[argv.index("--out") + 1], "w").write(canonical(manifest) + "\n")
        print("wrote", argv[argv.index("--out") + 1])
    print("replay:", manifest["deterministic_replay_sha256"])
    return 0


if __name__ == "__main__":
    sys.exit(main(sys.argv[1:]))
